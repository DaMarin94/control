...
- **Arquitectura en dos capas.** Primitiva reutilizable de charting **`components/ui/chart.tsx`** (Recharts v3 — instalado con `pnpm add`, no npm — themeada con tokens del DS; exporta `ChartContainer`/`ChartTooltipContent`/`ChartLegend`), pensada para reusarse en cualquier gráfico futuro. El módulo de feature **`components/charts/annual-chart-widget.tsx`** la compone, pero **ya no es un widget único con toggle**: exporta **dos tarjetas autónomas** — **`IncomeExpenseCard`** (Forma 1 = `AreaChart` ingresos/gastos; props `year`/`chartHeight`/`showYearInHeader`) y **`ByCategoryCard`** (Forma 2 = `BarChart` apilado por `category.color`; props `year`/`chartHeight`). Cada tarjeta consume `useAnnual(year)` por su cuenta. Gráfico nuevo → reusar la primitiva, no rehacer el motor.
...
- **Puntos de uso.** Pantalla dedicada `app/(app)/anual/page.tsx` monta `IncomeExpenseCard` + `ByCategoryCard` apiladas; **el control de año (`YearStepper`) vive en el `.phead` de la página, no en las tarjetas** — el año es **estado local de la página** y gobierna ambas tarjetas. **NO va en la URL → no usa `useSearchParams()` ni `<Suspense>`** (a diferencia de `/mes`). El dashboard monta **solo `IncomeExpenseCard`** con `showYearInHeader=true` y año fijo. Link "Anual" en el sidebar, activo por `startsWith("/anual")`.
- **Dedupe, no doble fetch.** En `/anual` se invoca `useAnnual(year)` varias veces (cada tarjeta + la página para `earliestYear`); React Query lo resuelve como **una sola request** por compartir la key `["annual", year]`. No alarmarse por ver varios `useAnnual` en el árbol.
