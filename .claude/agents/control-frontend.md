...
- **`delete-recurring-dialog`** — eliminación **sin opciones**: `fromCurrentMonth` siempre `true`, pivote = mes visualizado (RF-MF-004). El checkbox "Eliminar también desde este mes" fue **quitado**; ver el gotcha en la sección Fase 3.
...
- **GOTCHA — acordeón animado (altura) + drag: deshabilitar la transición de altura mientras dnd-kit mide/arrastra.** Si un ítem sortable es un acordeón cuyo cuerpo anima `grid-template-rows 0fr↔1fr`, dnd-kit usa alturas desactualizadas y los ítems se enciman/achatan. En `/mes`: en modo orden el colapso es **instantáneo** (prop `noTransition` en `AccordionSection`, activada por `SortableSection` cuando `isOrderMode`) + `measuring={{ droppable: { strategy: MeasuringStrategy.Always } }}` en el `DndContext` como defensa de profundidad para re-medir durante el drag.
- **GOTCHA — usar `CSS.Translate.toString`, no `CSS.Transform.toString`, para el ítem sortable.** Con `verticalListSortingStrategy` el `transform` de `useSortable` incluye `scaleX/scaleY` cuando los vecinos difieren en altura; `CSS.Transform.toString` aplica ese escalado y **achata** el ítem activo al intercambiarse. `CSS.Translate.toString(transform)` (solo traslación x/y) lo evita. Aplica a toda lista sortable de ítems de altura variable.
...
- **Gotchas reusables (Recharts v3 + Tailwind v4 + DS):**
  ...
  - **Pila apilada de gastos (modo "Por categoría" de `IncomeExpenseCard`, RF-REP-006):** la vista por categoría es **solo gastos** — un único `stackId` que apila las bandas de `data.categories[...]` (el mismo array que la Forma 2, `by-category`). No descompone ingresos, no hay segunda pila ni prefijos de dataKey: reusar el desglose de gastos existente.
  - **Selector de modo = dos tabs underline (RF-REP-006).** El indicador deslizante bajo el tab activo se posiciona **midiendo `offsetLeft`/`offsetWidth` del tab activo** con un `ref` + `useEffect` (recalcular al cambiar de modo / al montar), **no** con CSS puro: los labels ("Total" / "Por categoría") tienen anchos distintos, así que un slider de ancho fijo o `translateX` constante no calza. Cualquier tab-bar con indicador deslizante y labels de ancho variable necesita esta medición por ref.
...
- **GOTCHA — RN-019 tiene UN único punto de implementación en el frontend: `src/lib/movements.ts`.** Como `/mes` recalcula totales/subtotales en cliente (por el filtrado client-side de arriba), la regla de imputación a totales (RN-019, en `requirements.md`) quedaría duplicada respecto del backend. Está nucleada en `lib/movements.ts`: `sumMovementTotals(items) → { expense, income }` (magnitud al bucket del `type`) y `groupSubtotalCents(items) → number` (neto con signo de presentación, income − expense; delega en el anterior). **Todo componente que calcule totales o subtotales de movimientos importa de ahí — no reimplementes la suma inline.**
- **GOTCHA — `MovementItem.amountCents` puede ser NEGATIVO** (calculados `EXPENSE` lo son). Al bucketear por tipo **siempre `Math.abs()`** (lo hace `sumMovementTotals`); sumar el `amountCents` crudo con signo resta el gasto en vez de sumarlo y divergís del dashboard (fue la causa raíz del bug E1).
...
- **Enrutar SIEMPRE por `calculated.sourceType`** (`fijo` | `unico` | `cuota`) — NO asumir fijo (1.1.8). `sourceChainId` puede ser **null** (lo es en único/cuota). `useCalculated` elige el endpoint por `sourceType`: `fijo` → `/recurring`, `unico` → `/transactions`, `cuota` → `/installments` (POST y PATCH); el borrado reutiliza `deleteRecurring` (`DELETE /recurring/:id`) en los tres.
- **El calculado NO elige tipo: se deriva del signo+monto final** (`>0 → INCOME`, `≤0 → EXPENSE`). El form (`calculated-form.tsx`) **no tiene** selector Gasto/Ingreso; tiene operador + operando + signo + preview en vivo con badge de tipo derivado. **No enviar `type`** en ningún body. `CreateCalculatedRequest` lleva `startMonth` solo para origen fijo (único/cuota lo derivan en el backend).
- **Creación SOLO desde la acción "crear movimiento desde este"** (kebab) sobre un ítem **no calculado** (fijo, único o cuota) en `/mes`. No aparece sobre un ítem que ya es calculado. No hay tab "calculado" en el modal de carga; no inventarlo. Ícono en `docs/design.md`.
...
- **Borrado por `sourceType`:** fijo → `DeleteRecurringDialog` con opciones de mes (boundary); único/cuota → `variant="calculated-simple"`, confirmación directa (borrado total, RF-MCALC-009). Editar va SIEMPRE por `useCalculated.updateCalculated`; invalidar `["movements"]` por prefijo.
- **Padre/hijo:** chip de calculado + "desde {Origen}" si es hijo (`movement.calculated`), indicador de padre si `hasCalculated`, callout en el diálogo de borrado si `hasCalculated`. Detalle visual en `docs/design.md`.
