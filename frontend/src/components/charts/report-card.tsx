...
// ─── Popover de filtro de categorías ─────────────────────────────────────────

interface CategoryFilterPopoverProps {
  /** Ids de categorías seleccionadas. null = todas. */
...
function CategoryFilterPopover({
  ...
  // null = todas; si hay lista, solo las que existen en allCategories
  const effectiveIds: Set<string> =
    selectedIds === null
      ? new Set(allCategories.map((c) => c.id))
      : new Set(selectedIds.filter((id) => allCategories.some((c) => c.id === id)));

  const allSelected = allCategories.every((c) => effectiveIds.has(c.id));
  ...
  function toggleAll() {
    if (allSelected) {
      // Destildar todas → lista vacía (filtro vacío)
      onSelectionChange([]);
    } else {
...
function FilterButton({ selectedIds, totalCategories, isOpen, buttonRef, onClick }: FilterButtonProps) {
