...
// ─── Tests: RF-REP-015 — handleProjectFixedChange en la página ────────────────

describe("ReportesPage — handleProjectFixedChange (RF-REP-015)", () => {
  const incomeCard: ReportCardConfig = {
    id: "card-1",
    type: "income-expense",
    year: 2026,
    categoryIds: null,
  ...
  it("al activar proyección (toggle on), persiste projectFixed=true en la card", async () => {
    const setPreferences = vi.fn().mockResolvedValue({ success: true });
    makePreferencesHook([incomeCard], setPreferences);
    renderPage();

    // El toggle Proyección debe estar presente (callback proporcionado por la página)
    const toggleBtn = screen.getByRole("button", { name: /proyección de fijos a futuro/i });
    expect(toggleBtn).toBeInTheDocument();

    // Inicialmente off (sin projectFixed en la config)
    ...
    const callArg = setPreferences.mock.calls[0]?.[0] as UserPreferences;
    expect(callArg.reports?.[0]?.projectFixed).toBe(true);
  });

  it("al desactivar proyección (toggle off), omite el campo projectFixed del blob (back-compat)", async () => {
    const setPreferences = vi.fn().mockResolvedValue({ success: true });
    const cardWithProjection: ReportCardConfig = {
      ...incomeCard,
      projectFixed: true,
    };
    makePreferencesHook([cardWithProjection], setPreferences);
    renderPage();
    ...
    const callArg = setPreferences.mock.calls[0]?.[0] as UserPreferences;
    // El campo debe estar AUSENTE cuando está apagado (back-compat: undefined/no key)
    expect(callArg.reports?.[0]?.projectFixed).toBeUndefined();
  });

  it("back-compat: card sin projectFixed → toggle muestra aria-pressed=false (default off)", () => {
    makePreferencesHook([incomeCard]);
    renderPage();
