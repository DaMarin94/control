...
- Los criterios de arriba describen el dato **real** de ambos tipos. Una card `income-expense` o `by-category` puede además sumar los **movimientos simulados** al tramo que alcanza el horizonte —**desde el mes en curso**, que queda mixto— cuando la propia card lo habilita (RF-REP-017, apagado por defecto).
...
| Campo | Detalle |
|---|---|
| **Descripción** | Las cards `income-expense` (Ingresos vs Gastos) y `by-category` (Gastos por categoría) exponen un **toggle que habilita los movimientos simulados** (módulo 3.15) en el tramo del año que la card muestra que cae **dentro del horizonte** —es decir, **desde el mes en curso** en adelante—. Es un control **por card**, persistido junto al resto de su config (clave `reports`, RF-REP-004) —mismo patrón que el techo de color de `unique-grid` (RF-REP-010)—, **apagado por defecto**: una card sin el toggle habilitado grafica **solo lo real**. Ninguna otra card lo expone; el dashboard tampoco. |
| **Actor** | Usuario autenticado |
| **Prioridad** | Media |
...
- **Desde el mes en curso.** El simulado entra en los meses **dentro del horizonte** de cada simulación (RF-SIM-002), que arranca en el **mes en curso**. Los **meses pasados** de la card no cambian nunca.
...
- **Tramo alcanzable.** La navegación de año topa hacia adelante en el **año en curso** (RF-REP-002), así que el único tramo visible es `A .. diciembre del año en curso`; la extensión del horizonte a `A+6` que cae en el año siguiente **no es alcanzable** desde `/reportes`.
...
- [ ] Habilitado, los simulados entran **solo** en los meses **dentro del horizonte** (RF-SIM-002), **desde el mes en curso**; los meses pasados de la card no cambian.
...
La simulación proyecta, **desde el mes en curso en adelante**, el comportamiento de **una categoría** a partir de su historia reciente, y lo muestra como movimientos en la Vista del mes (`/mes`, RF-SIM-003) y —**opt-in por card**— en las cards de Ingresos vs Gastos y Gastos por categoría de `/reportes` (RF-REP-017). Lo que muestra en cada mes es el **remanente**: lo que falta para llegar a lo proyectado, descontando los únicos reales que ese mes ya tiene (RN-028).

Su alcance son **únicamente los movimientos únicos**. Los fijos y las cuotas ya se calculan on-the-fly en cualquier mes (RN-006): navegando a un mes del horizonte, los fijos activos y las cuotas en tramo **ya aparecen**. El único hueco son los **Únicos** — simular fijos o cuotas duplicaría lo que el sistema ya sabe.
...
4. Las simulaciones quedan activas: los meses del horizonte pasan a mostrar su movimiento simulado (RF-SIM-002, RF-SIM-003).
...
- **Horizonte — desde el mes en curso `A` hasta diciembre del año en curso**; si ese tramo tiene **menos de 6 meses**, se extiende hasta `A+6` (el tramo simulado nunca es menor a 6 meses). Los **meses pasados nunca se simulan**.
- **El monto es un remanente.** En **cada** mes del horizonte: `remanente = proyección(mes) − total real con signo de los únicos de esa categoría en ese mes`, tomando el real con el mismo criterio que la serie de ajuste. La simulación significa **"lo que falta para llegar a lo proyectado"**, no un monto que se suma encima de lo ya cargado. Aplica igual al **mes en curso** (que ya tiene sus únicos reales) y a un **mes futuro** con únicos cargados por adelantado.
...
- **Recálculo permanente:** la simulación no congela ningún número. El cálculo se rehace en cada lectura con la ventana, el horizonte y los movimientos reales **vigentes en ese momento**; cargar un único de la categoría en un mes del horizonte **encoge el remanente de ese mes** en el acto, y al avanzar el calendario un mes simulado queda atrás y **deja de derivarse solo**, sin dejar residuo.
...
- [ ] El horizonte va **del mes en curso** a **diciembre del año en curso**, extendido a `A+6` cuando ese tramo queda por debajo de 6 meses. Los **meses pasados nunca** llevan movimientos simulados.
...
**Criterios de aceptación:**
- [ ] Cada simulación activa aporta **como máximo un** movimiento simulado por mes del horizonte —**mes en curso incluido**—, en la sección **Únicos**; un mes cuyo remanente redondea a 0 o cambia de signo no lleva ninguno (RF-SIM-002).
...
4. Los movimientos simulados de esa categoría desaparecen de todos los meses del horizonte y los totales se recalculan sin ellos.
...
- [ ] Eliminar una simulación **deja de derivar** sus movimientos simulados en **todos** los meses del horizonte de inmediato; los totales, subtotales y contadores de esos meses se recalculan sin ellos.
...
| Movimiento simulado | Movimiento **derivado al vuelo** que una simulación de categoría aporta a la sección Únicos de un mes del horizonte (**mes en curso incluido**). Es un **remanente**: lo que falta para llegar a lo proyectado, descontando los únicos reales del mes. No existe como fila y no es editable ni accionable. Se ve **siempre** en `/mes` y, **opt-in por card**, en `/reportes` (RF-REP-017). Su dirección y magnitud salen del cálculo (RN-028). Ver RF-SIM-003. |
...
| Simulación de categoría | Configuración del usuario (usuario + categoría) que proyecta, **desde el mes en curso** hasta el fin del horizonte, el total de **movimientos únicos** de esa categoría, por regresión lineal sobre los 12 meses anteriores. Se crea (de a una o varias categorías) y se elimina desde el filtro de la sección Únicos de `/mes`; no se edita y eliminarla es definitivo. Ver módulo 3.15 (RF-SIM-001..004), RF-REP-017, RN-028/029. |
