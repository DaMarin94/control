...
> **Alcance:** los tipos de reporte descritos en RF-REP-001 (ingresos/gastos y apilado por categoría de gastos), RF-REP-010 (grilla anual de gastos Únicos día × mes), RF-REP-011 (gantt anual de gastos en Cuotas), RF-REP-012 (líneas de Inflación vs Ingresos) y RF-REP-013 (Detalle histórico de gastos fijos). La card de Ingresos vs Gastos admite además filtros por tipo / dirección / categoría (RF-REP-014); ella y la de Gastos por categoría pueden incluir los **movimientos simulados** en el tramo alcanzado por las simulaciones —como mínimo desde el mes en curso—, opt-in por card (RF-REP-017). Otros tipos de reporte/gráfico (torta, barras de comparación) quedan fuera de alcance (ver sección 6).
...
- **Excepción a "mes sin datos = cero":** el criterio vale para los dos tipos de este RF. La card `fixed-evolution` (RF-REP-013) grafica la **ausencia como hueco, nunca como cero** —una línea suya puede valer 0 de verdad (RN-018)—; esa excepción es intencional y no se "corrige".
- Los criterios de arriba describen el dato **real** de ambos tipos. Una card `income-expense` o `by-category` puede además sumar los **movimientos simulados** al tramo que alcanzan las simulaciones —**como mínimo desde el mes en curso**, que queda mixto— cuando la propia card lo habilita (RF-REP-017, apagado por defecto).
...
#### RF-REP-013 — Reporte de Detalle histórico de gastos fijos (gráfico de líneas)
...
- **Modo de visualización (3 lecturas del mismo dato, aplicadas por línea):**
  ...
  - **Variación % nominal** — variación del monto de **cada fijo** respecto de **su pago anterior**, en puntos porcentuales. Sin punto si esa línea no tiene pago anterior conocido o si ese pago valió cero.
  - **Variación % ajustada por inflación** — la misma variación descontando la **inflación acumulada del tramo** entre los dos pagos (IPC nacional, RF-IPC-001); sin punto si no hay pago anterior o si falta el dato de IPC de **alguno** de los meses del tramo.
  - El **ajuste por inflación aplica solo a los modos de variación**, nunca al modo de montos.
- **La variación compara pagos consecutivos de la línea, no meses consecutivos del calendario.** El término de comparación de un punto es **la aparición anterior de ese mismo fijo**, esté donde esté:
  - Para un fijo **mensual** el pago anterior es el mes anterior.
  - Para un fijo de **frecuencia > 1** son los dos pagos consecutivos, salteando los meses sin aparición: un bimestral compara contra hace dos meses; un anual, contra hace doce.
  - Los **meses anulados** (`RecurringSkip`) no son pago y **no cortan la cadena**: la comparación va contra el último pago real previo.
  - El pago anterior **puede caer fuera del rango visible** y se usa igual, de modo que el **primer punto visible también tiene variación**.
- **La inflación del tramo es todo o nada.** El ajuste descuenta el IPC **acumulado compuesto** de los meses que van del pago anterior al pago del punto. Si falta el dato de IPC de **cualquiera** de esos meses, el punto **no tiene variación ajustada**: descontar solo una parte de la inflación del tramo daría un resultado engañosamente favorable. El mismo punto **sí** muestra su variación **nominal**.
...
**Comportamiento de la línea por caso de dominio:**
...
- [ ] Los modos de variación comparan cada punto contra el **pago anterior de esa misma línea**, no contra el mes anterior del calendario: un fijo de **frecuencia > 1** tiene variación en cada aparición (salteando los meses sin pago) y los meses **anulados no cortan la cadena** (se compara contra el último pago real previo).
- [ ] El **pago anterior se usa aunque caiga fuera del rango visible**: el primer punto graficado también tiene variación.
- [ ] La **variación ajustada** descuenta la **inflación acumulada del tramo** entre los dos pagos y queda **sin punto si falta el IPC de alguno** de los meses de ese tramo (nunca se descuenta inflación parcial); ese mismo punto conserva su variación **nominal**.
...
- La card necesita el **monto mensual de cada cadena de fijo a lo largo del rango** (reconstruible de la cadena de splits `Recurring`, RN-005). El contrato del endpoint se fija en implementación (`docs/data-model.md`, §Contrato del detalle histórico de Fijos); este RF no lo prescribe.
- **Por qué la variación se mide entre pagos consecutivos:** comparar contra el mes anterior del calendario deja **sin ningún punto** a los fijos de frecuencia > 1 —su mes anterior nunca tiene monto—, justo los fijos donde más interesa leer cuánto aumentaron. Medir entre pagos es aditivo: para un fijo **mensual** da exactamente lo mismo, porque su pago anterior *es* el mes anterior.
...
- **Relación con RF-REP-013:** la proyección de fijos y el Detalle histórico de gastos fijos (RF-REP-013) son ambas miradas sobre los fijos; aquí es la mirada de **totales** proyectada a futuro, mientras que RF-REP-013 es la mirada **desagregada, una línea por fijo**, sobre montos conocidos.
...
| Campo | Detalle |
|---|---|
| **Descripción** | Cada card de `/reportes` —los seis tipos: income-expense (RF-REP-005), by-category (RF-REP-006), unique-grid (RF-REP-010), installment-gantt (RF-REP-011), inflation-income (RF-REP-012), fixed-evolution (RF-REP-013)— **y** el widget income-expense del Dashboard exponen en su cabecera un botón de **refrescar** que vuelve a pedir al backend **solo los datos de esa card**. El refetch es **independiente por card**: refrescar una no afecta a las demás. **Backend sin cambios**: reusa el mismo endpoint de datos de la card. El detalle visual (forma, ubicación, ícono, estado de carga) está en `docs/design.md`. |
| **Actor** | Usuario autenticado |
| **Prioridad** | Baja |
| **Precondiciones** | El usuario está en `/reportes` (o en el Dashboard, para su widget income-expense). |

**Criterios de aceptación:**
- [ ] Cada uno de los seis tipos de card de `/reportes` y el widget income-expense del Dashboard exponen un botón de **refrescar** en su cabecera.
...
| Campo | Detalle |
|---|---|
| **Descripción** | Las cards `income-expense` (Ingresos vs Gastos) y `by-category` (Gastos por categoría) exponen un **toggle que habilita los movimientos simulados** (módulo 3.15) en el tramo del año que la card muestra que cae **dentro de la unión de tramos** de las simulaciones del usuario —que arranca **como mínimo desde el mes en curso**—. Es un control **por card**, persistido junto al resto de su config (clave `reports`, RF-REP-004) —mismo patrón que el techo de color de `unique-grid` (RF-REP-010)—, **apagado por defecto**: una card sin el toggle habilitado grafica **solo lo real**. Ninguna otra card lo expone; el dashboard tampoco. |
| **Actor** | Usuario autenticado |
| **Prioridad** | Media |
...
| `fixed-evolution` | No | Grafica una **línea por gasto fijo**; el simulado es un único. |
| `inflation-income` | No | Grafica **variaciones porcentuales contra el IPC**, no montos. |

**Comportamiento con el toggle habilitado:**

- **Unión de tramos, desde el mes en curso como mínimo.** El simulado entra en los meses **dentro del tramo de cada simulación** (RF-SIM-002); la card, en conjunto, alcanza la **unión** de esos tramos, que arranca como mínimo en el **mes en curso** — una simulación nunca alcanza meses pasados. Los **meses pasados** de la card no cambian nunca.
- **El mes en curso de la card es un mes mixto:** su valor es lo real a la fecha **más** el remanente simulado (RF-SIM-002), y entra en la capa simulada como cualquier otro mes del tramo.
- **Tramo alcanzable.** La navegación de año topa hacia adelante en el **año en curso** (RF-REP-002), así que lo visible de la unión de tramos es `A .. diciembre del año en curso`; el tramo de cualquier simulación cuyo `endMonth` se extienda al año siguiente (`startMonth + 6`, RF-SIM-002) **no es alcanzable** desde `/reportes`.
...
- **Límites (RF-LIM-003).** Las **marcas pasivas** evalúan siempre el **dato que la card muestra**: con el toggle habilitado, el valor **con** simulados; apagado, el real. No se agregan keys — las de la card (`reporte.ie.*`, `reporte.cat.*`) no cambian. Los reportes no tienen límites activos (RF-LIM-004). **Consecuencia:** un límite de alcance **"mes en curso"** puede quedar marcado en `/reportes` sobre **dato estimado** (real a la fecha + remanente), porque ese es el dato que la card muestra.
- **Simulación pausada.** Una simulación pausada (RF-SIM-002) no deriva movimientos: no aporta nada a la card y eso no es un error.

**Disponibilidad del control:** el toggle está **siempre presente** en las dos cards y se ofrece **deshabilitado con el motivo visible** —nunca oculto— en dos casos: el usuario **no tiene ninguna simulación**, o el año que la card muestra **ya pasó** (`año < año en curso`) — en este segundo caso el motivo explica que las simulaciones **no alcanzan meses pasados**. El **año en curso nunca se deshabilita por el año**: siempre tiene al menos un mes simulable —el mes en curso—, incluso en diciembre. La **precedencia** entre los dos motivos: "no tenés ninguna simulación" gana sobre el motivo de año. Deshabilitar el control **no apaga el valor persistido**: al volver a un año alcanzado por alguna simulación, la card retoma el estado que tenía.
...
- [ ] Habilitado, los simulados entran **solo** en los meses **dentro del tramo de cada simulación** (RF-SIM-002); la card alcanza la **unión** de esos tramos, que arranca **como mínimo en el mes en curso**; los meses pasados de la card no cambian.
...
- [ ] El toggle se ofrece **deshabilitado con el motivo visible** (nunca oculto) sin simulaciones del usuario o en un **año anterior al año en curso**, **sin perder** el valor persistido. El **año en curso siempre lo ofrece habilitado** (con simulaciones), incluso en diciembre.
- [ ] La card **no** muestra aviso textual de composición: la distinción es visual, dentro del gráfico.

**Notas:**
- **Asimetría con RF-REP-015 (proyección de fijos, capacidad de backend sin UI).** Las dos extienden el tramo futuro de una card y solo una tiene control de usuario; la diferencia es **quién declara ese futuro**. La simulación es una **elección explícita del usuario** —eligió la categoría (RF-SIM-001) y ya ve su resultado en `/mes` (RF-SIM-003)—, con tramo acotado, y el toggle solo lleva a `/reportes` un dato que el usuario pidió. La proyección de fijos es una **estimación del sistema**, de horizonte ilimitado, que nadie declaró. Habilitar este toggle **no** activa la proyección de fijos: los meses futuros de la card siguen valiendo lo que aportan los fijos activos y las cuotas en tramo (RN-006), más los simulados.
...
La simulación proyecta, a lo largo de un **tramo propio** que arranca en el mes desde el que se la crea y **nunca alcanza meses pasados**, el comportamiento de **una categoría** a partir de su historia reciente, y lo muestra como movimientos en la Vista del mes (`/mes`, RF-SIM-003) y —**opt-in por card**— en las cards de Ingresos vs Gastos y Gastos por categoría de `/reportes` (RF-REP-017). Lo que muestra en cada mes es el **remanente**: lo que falta para llegar a lo proyectado, descontando los únicos reales que ese mes ya tiene (RN-028).

Su alcance son **únicamente los movimientos únicos**. Los fijos y las cuotas ya se calculan on-the-fly en cualquier mes (RN-006): navegando a un mes del tramo de una simulación, los fijos activos y las cuotas en tramo **ya aparecen**. El único hueco son los **Únicos** — simular fijos o cuotas duplicaría lo que el sistema ya sabe.

La simulación **no genera filas**: lo que persiste es su configuración (usuario + categoría) y su **tramo** (RN-028/029); los movimientos simulados se **derivan al vuelo** en cada lectura, igual que los fijos y las cuotas.
...
| Campo | Detalle |
|---|---|
| **Descripción** | Desde el **disparador de filtro de la sección Únicos** de `/mes` (RF-VM-006), la acción **"Simular categoría"** crea simulaciones sobre **una o varias** categorías en **una sola operación**, todas **ancladas al mes visualizado** (RN-028). Lo que persiste es la configuración (usuario + categoría) y el **tramo** resuelto al crear: ni monto ni dirección se ingresan — salen del cálculo (RF-SIM-002). Un usuario puede tener **varias simulaciones a la vez**, como máximo **una por categoría**. |
| **Actor** | Usuario autenticado |
| **Prioridad** | Media |
...
2. El sistema ofrece **solo las categorías simulables**: las del catálogo activo con **3 o más meses con únicos** en la ventana histórica que **no** tienen ya una simulación.
3. El usuario elige **una o varias** categorías y confirma.
4. Las simulaciones quedan activas: los meses de su tramo pasan a mostrar su movimiento simulado (RF-SIM-002, RF-SIM-003).

**Flujos alternativos:**
- *A1 — El usuario cancela:* no se crea nada.
- *A2 — Ninguna categoría es simulable:* la lista queda vacía y el sistema **explica la causa** (ver criterios).
- *A4 — Alguna categoría de la selección falla:* la operación **no es atómica** — las que se pudieron crear **quedan creadas** y el usuario se entera de **cuáles fallaron y por qué**, con la selección todavía abierta para reintentar solo esas. Que fallen **todas** tampoco es un error duro del alta: mismo desenlace, sin nada creado.
...
- [ ] Se eligen **una o varias categorías** y se confirman en **una sola operación**. Pueden coexistir **varias simulaciones**, a lo sumo **una por categoría**.
- [ ] El alta de varias es **tolerante a fallo parcial**: una categoría que falla **no impide** las demás, cada fallo se comunica **por categoría** con su motivo, y lo creado **no se revierte**.
- [ ] **Solo se ofrecen las categorías simulables:** las que tienen **menos de 3 meses con movimientos únicos** en la ventana histórica (RF-SIM-002) y las que **ya están simuladas** **no se listan**. El criterio se enuncia en una **nota de elegibilidad** al pie de la lista, para que la ausencia de una categoría sea explicable sin mostrarla.
- [ ] El **universo se congela al abrir** el selector: ninguna categoría desaparece de la lista mientras el usuario está eligiendo, aunque los datos cambien detrás.
- [ ] **Lista vacía — se explica la causa.** Cuando no queda ninguna categoría simulable, el mensaje distingue **cuatro** situaciones: el usuario **no tiene categorías activas**; **ninguna llega a 3 meses** con datos; **ya están simuladas todas** las que se pueden simular; o **mixto** (las que tienen datos ya están simuladas y a las demás les faltan meses).
- [ ] El universo del que sale la lista es el **catálogo de categorías activas** del usuario (las eliminadas no se ofrecen, RN-007), **no** las categorías presentes en el mes visualizado: la simulación se apoya en la ventana histórica, que no depende del mes navegado.
- [ ] Lo que persiste es la **simulación** (usuario + categoría + tramo, RN-028). **No se crean movimientos ni filas** de ningún tipo.
- [ ] El **tramo se ancla al mes visualizado** al crear y **no revive meses pasados**: crear parado en un mes anterior al mes en curso equivale a crear desde el mes en curso (RN-028). Todas las categorías del mismo alta comparten el ancla.
- [ ] La simulación es editable en **un solo eje: el fin del tramo**, que se extiende en incrementos fijos (RF-SIM-005). El `startMonth` y el monto simulado **no** se ajustan a mano; **crear** (este RF), **extender** (RF-SIM-005) y **eliminar** (RF-SIM-004) son sus tres únicas operaciones.
...
- El filtrado por elegibilidad es **de presentación**: el contrato de candidatas trae el catálogo completo con los datos que permiten decidir y distinguir la causa del vacío (`docs/data-model.md`, §Simulación de categoría).
...
| Campo | Detalle |
|---|---|
| **Descripción** | El monto y la dirección del movimiento simulado de cada mes del tramo de una simulación se derivan de una **regresión lineal por mínimos cuadrados** sobre el **total mensual con signo** de los movimientos únicos de la categoría en los **12 meses anteriores** al mes en curso, **menos lo que esa categoría ya lleva realmente cargado en el mes**: el simulado es el **remanente**, "lo que falta para llegar a lo proyectado". El cálculo es **on-the-fly en cada lectura** (RN-006): no se persiste ningún monto. |
| **Actor** | Sistema |
| **Prioridad** | Alta |
...
- **Eje:** los meses de la ventana ocupan las posiciones `1..12` (`A−12` = 1 … `A−1` = 12); el **mes en curso ocupa la 13** aunque no entre a la regresión, y cada mes siguiente la que le sigue (`A+1` = 14, `A+2` = 15, …). La recta se ajusta sobre los 12 puntos y se **extrapola** evaluándola en la posición del mes a simular, **incluido el mes en curso**.
...
- **Tramo — propio de cada simulación, fijado al crearla y persistido.** `startMonth` = `max(mes desde el que se la crea, mes en curso)`; `endMonth` = **diciembre del año de `startMonth`**, extendido a `startMonth + 6` si ese tramo queda por debajo de 6 meses (nunca es menor a 6 meses). Ejemplo: creada en octubre de 2026, `oct..dic` son 3 meses, así que `endMonth` = **abril de 2027**.
- **Arranque efectivo = `max(startMonth, mes en curso)`.** El `startMonth` guardado no cambia nunca, pero un tramo que ya arrancó **no revive meses pasados**. Un mes lleva movimiento simulado si `arranque efectivo <= mes <= endMonth`, **evaluado simulación por simulación**: dos simulaciones pueden cubrir tramos distintos. Los **meses pasados nunca se simulan**.
- **La ventana, la pausa y el remanente se resuelven contra el mes en curso**, no contra el `startMonth`: el tramo dice **en qué meses** aparece el simulado, no con qué datos se calcula.
- **El monto es un remanente.** En **cada** mes del tramo: `remanente = proyección(mes) − total real con signo de los únicos de esa categoría en ese mes`, tomando el real con el mismo criterio que la serie de ajuste. La simulación significa **"lo que falta para llegar a lo proyectado"**, no un monto que se suma encima de lo ya cargado. Aplica igual al **mes en curso** (que ya tiene sus únicos reales) y a un **mes futuro** con únicos cargados por adelantado.
- **Dirección = resultado del cálculo.** El **signo del remanente** define el tipo del movimiento simulado: **negativo → gasto**; **positivo → ingreso**. La magnitud es `|remanente|`. El usuario elige **solo la categoría**, nunca la dirección; una misma simulación puede resultar gasto en un mes e ingreso en otro.
- **Redondeo y cancelación:** el remanente se redondea a **centavos enteros** (RN-002). Ese mes **no genera movimiento simulado** —no se muestra ninguna fila— en dos casos: si **redondea a 0**, y si **cambia de signo respecto de la proyección**. Lo segundo es lo que impide que un **gasto proyectado se dé vuelta como ingreso** porque la categoría ya gastó de más ese mes: la simulación se agota en cero, no se invierte.
- **Recálculo permanente:** la simulación no congela ningún número. El **tramo** ya quedó fijado y persistido al crearla (RN-028/029); lo que se rehace en cada lectura es el cálculo, con la **ventana** y los movimientos reales **vigentes en ese momento**: cargar un único de la categoría en un mes del tramo **encoge el remanente de ese mes** en el acto, y al avanzar el calendario un mes simulado queda atrás y **deja de derivarse solo**, sin dejar residuo.
...
- [ ] La **proyección** de un mes es la **recta de mínimos cuadrados evaluada en la posición de ese mes**, con el mes en curso ocupando la posición 13 aunque no entre a la regresión.
- [ ] Con **menos de 3 meses con únicos** en la ventana no se produce ningún movimiento simulado.
- [ ] El monto simulado de un mes es el **remanente** (proyección menos el total real con signo de los únicos de la categoría en ese mes), tanto en el **mes en curso** como en un mes futuro que ya tenga únicos cargados.
- [ ] El **signo del remanente** define el tipo: negativo → **gasto**, positivo → **ingreso**; la magnitud del movimiento es el valor absoluto.
- [ ] Un remanente que **redondea a 0 centavos** o que **cambia de signo respecto de la proyección** **no genera** movimiento simulado ese mes.
- [ ] El **tramo de cada simulación** va de su **arranque efectivo** a su `endMonth` persistido —diciembre del año de `startMonth`, extendido a `startMonth + 6` cuando ese tramo queda por debajo de 6 meses—, evaluado **por simulación**. Los **meses pasados nunca** llevan movimientos simulados.
- [ ] El cálculo es **on-the-fly**: no se persiste ningún monto, y navegar dos veces al mismo mes recalcula con los datos vigentes.
...
**Criterios de aceptación:**
- [ ] Cada simulación activa aporta **como máximo un** movimiento simulado por mes **de su tramo** —**mes en curso incluido**—, en la sección **Únicos**; un mes cuyo remanente redondea a 0 o cambia de signo no lleva ninguno (RF-SIM-002).
- [ ] El **mes en curso** es un **mes mixto**: lista sus únicos reales y, además, el **remanente** de cada simulación que alcance (RF-SIM-002). El movimiento simulado no duplica lo ya cargado — lo descuenta.
...
- [ ] **Límites (RF-LIM-003, RF-LIM-004):** los datos que `/mes` emite ya lo incluyen, así que las **marcas pasivas** lo evalúan como a cualquier dato del mes y la **alerta activa** proyecta sobre una base que lo contiene. Como el **mes en curso** puede llevar simulados, esto alcanza a las **cuatro** formas de carga: el único se chequea contra el mes al que va, y fijos, cuotas y calculados contra el mes en curso. **Consecuencia:** un límite de alcance **"mes en curso"** (RF-LIM-003 / RF-LIM-004) puede marcar o avisar sobre un dato que incluye el remanente simulado, es decir sobre **dato estimado**. Es el dato que la superficie muestra, y esa es la regla (RN-022).
...
4. Los movimientos simulados de esa categoría desaparecen de todos los meses que alcanzaba y los totales se recalculan sin ellos.
...
- [ ] Eliminar una simulación **deja de derivar** sus movimientos simulados en **todos** los meses de su tramo de inmediato; los totales, subtotales y contadores de esos meses se recalculan sin ellos.
...
- **La simulación de categoría queda entera fuera del historial de cambios.** Ni su creación, ni su extensión (RF-SIM-005) ni su eliminación se registran, y la eliminación no es deshacible. Es la excepción que RN-027 exige documentar en el propio RF; ninguna funcionalidad fuera de este módulo la hereda.
- **Por qué es admisible:** una simulación **no contiene datos del usuario**. Lo único que persiste es la elección de una categoría y su tramo (RN-029); el monto y la dirección se derivan al vuelo en cada lectura (RN-028) y no hay nada cargado a mano que perder. Rehacerla cuesta exactamente lo mismo que hacerla la primera vez, desde el mismo punto de entrada. La red de seguridad que RN-027 protege —no perder trabajo de carga— no tiene nada que proteger acá.

---

#### RF-SIM-005 — Extender el tramo de una simulación

| Campo | Detalle |
|---|---|
| **Descripción** | Corre el `endMonth` de una simulación **hacia adelante**, en un incremento fijo de **1, 3, 6 o 12 meses** sumado al `endMonth` vigente. El `startMonth` **nunca se toca**. El disparador se ofrece **solo** cuando el mes visualizado en `/mes` es **exactamente** el `endMonth` de esa simulación; en cualquier otro mes esa fila no lo ofrece. |
| **Actor** | Usuario autenticado |
| **Prioridad** | Media |
| **Precondiciones** | El usuario tiene una simulación cuyo `endMonth` coincide con el mes visualizado en `/mes`. |

**Flujo principal:**
1. Con el mes visualizado en `/mes` igual al `endMonth` de una simulación, el usuario abre el disparador de filtro de la sección **Únicos** y encuentra, en la fila de esa simulación, la acción **"Extender"**.
2. El sistema ofrece cuatro incrementos: **1, 3, 6 o 12 meses**.
3. El usuario elige uno.
4. El `endMonth` de la simulación se corre esa cantidad de meses hacia adelante; el popover **no se cierra**.

**Flujos alternativos:**
- *A1 — El usuario no elige ningún incremento:* la simulación queda intacta.
- *A2 — La extensión falla:* el `endMonth` queda como estaba y el usuario puede reintentar.

**Criterios de aceptación:**
- [ ] El disparador **"Extender"** aparece **únicamente** en la fila de la simulación cuyo `endMonth` es **exactamente** el mes visualizado en `/mes`; cambiar de mes lo mueve a la fila que corresponda o lo hace desaparecer si ninguna coincide.
- [ ] Los incrementos ofrecidos son **1, 3, 6 o 12 meses**, sumados al `endMonth` vigente; no hay otro valor posible.
- [ ] El `startMonth` **no se modifica nunca** por esta acción.
- [ ] El nuevo `endMonth` **no se re-deriva** con la regla de alta (RF-SIM-002/RN-028, `computeHorizonEndMonth`, exclusiva de la creación): es el `endMonth` vigente más el incremento elegido, **sin clampear** contra el mes en curso y **sin tope**.
- [ ] **Sin tope de extensión, por construcción:** al extender, ese mes deja de ser el `endMonth` de la simulación, así que el disparador deja de ofrecerse en ese lugar — repetir la acción exige navegar antes al nuevo último mes.
- [ ] Una simulación cuyo `endMonth` **ya pasó** se extiende **igual**, sin rama especial y sin ningún estado o cartel de "vencida" o "terminada" (esa marca no existe, RN-029).
- [ ] Una simulación **pausada** también ofrece extender, con la misma disponibilidad que Eliminar (RF-SIM-004) en pausa.
- [ ] La extensión **no pide confirmación**: es aditiva, no destructiva. Un toast comunica el éxito.
- [ ] Extender **no genera entrada de historial** (excepción a RN-027 documentada en RF-SIM-004), igual que crear (RF-SIM-001) y eliminar (RF-SIM-004) una simulación.

**Notas:**
- El shape de la entidad y el contrato del endpoint se fijan en implementación (`docs/data-model.md`, §Simulación de categoría).
- **Por qué no hay tope:** el propio disparador se auto-desactiva al extender —deja de cumplir su condición de aparición—, así que un tope adicional no protegería nada que la mecánica de UI no cubra ya.
...
| RN-028 | **Cálculo del movimiento simulado de una categoría (RF-SIM-002).** Con `A` = mes en curso: la serie de ajuste son los **12 meses anteriores** `[A−12 .. A−1]`, un punto por mes con el **total mensual con signo** de los movimientos **únicos** de la categoría (ingresos +, gastos −) en la **moneda default** del usuario, sin los anulados (RN-020) ni los eliminados (RN-026). Los meses **sin únicos de la categoría valen 0 y entran igual**: la ausencia es dato. El **mes en curso no entra a la regresión** (está incompleto y distorsionaría la tendencia). Sobre esos 12 puntos se ajusta una **regresión lineal por mínimos cuadrados** y se **extrapola** evaluándola en la posición del mes a simular (eje: `A−12` = 1 … `A−1` = 12, **mes en curso = 13**, `A+1` = 14, …). Se exige un **mínimo de 3 meses con únicos** en la ventana; por debajo no hay simulación. El **tramo** que la simulación proyecta es **propio de cada simulación** y se **persiste al crearla** (RN-029): `startMonth` = `max(mes desde el que se la crea, mes en curso)` —crear parado en un mes pasado equivale a crear desde el mes en curso— y `endMonth` = **diciembre del año de `startMonth`**, extendido a `startMonth + 6` cuando ese tramo queda por debajo de 6 meses; ese `endMonth` puede correrse después, hacia adelante, por la extensión (RF-SIM-005), sin volver a aplicar esta fórmula. En cada lectura el **arranque efectivo** es `max(startMonth, mes en curso)`: el `startMonth` guardado no cambia nunca, pero un tramo que ya arrancó **no revive meses pasados**. Un mes lleva el movimiento simulado de una simulación si `arranque efectivo <= mes <= endMonth`, **evaluado por simulación**: no hay un tramo único y dos simulaciones pueden cubrir meses distintos. Los **meses pasados nunca se simulan**. La **ventana de ajuste, el mínimo de 3 meses, la pausa y el remanente se resuelven siempre contra el mes en curso**, no contra el `startMonth`. El monto de **cada** mes del tramo es un **remanente** = `proyección(mes) − total real con signo de los únicos de esa categoría en ese mes` (el real, con el mismo criterio que la serie de ajuste): el simulado es **lo que falta para llegar a lo proyectado**, y descuenta los únicos ya cargados por igual en el mes en curso y en un mes futuro. El **signo del remanente define la dirección** (negativo → gasto, positivo → ingreso) y su valor absoluto, la magnitud: el usuario elige solo la categoría, nunca la dirección. El remanente se **redondea a centavos enteros** (RN-002) y ese mes **no genera movimiento** si redondea a **0** o si **cambia de signo respecto de la proyección** (lo real ya se pasó: un gasto proyectado nunca se da vuelta como ingreso simulado). El cálculo es **on-the-fly en cada lectura** (RN-006): no se persiste ningún monto y la ventana es siempre la vigente al momento de leer. |
| RN-029 | **Alcance y ciclo de vida de la simulación de categoría (RF-SIM-001, RF-SIM-003, RF-SIM-004, RF-SIM-005).** La simulación alcanza **solo movimientos únicos**: fijos y cuotas ya se derivan on-the-fly en cualquier mes (RN-006), así que el único hueco de un mes futuro son los Únicos. Persiste **la configuración y el tramo** —usuario, categoría, `startMonth` y `endMonth` (RN-028)—, con **a lo sumo una simulación por categoría** y varias a la vez; el alta admite **varias categorías en una sola operación**, todas ancladas al mismo mes. Es editable en **un solo eje: el fin del tramo**. El `startMonth` es **inmutable**, fijado al crear y sin ninguna operación que lo toque; el `endMonth` se puede **extender** hacia adelante en incrementos fijos de **1, 3, 6 o 12 meses** (RF-SIM-005), sin re-derivarse con la regla del alta y sin tope. El `endMonth` se **persiste en vez de derivarse** porque es un extremo **movible por simulación** —un valor que se puede correr— y no una función del mes en curso. Una simulación cuyo `endMonth` ya quedó atrás simplemente **no aporta**; **no existe estado ni marca de "terminada"**, y se extiende igual que cualquier otra. Cada simulación aporta **como máximo un** movimiento simulado por mes de **su** tramo (**mes en curso incluido**) a la sección **Únicos** de `/mes`, distinguible de los reales, que **suma a los totales del mes** (RF-VM-002, con la imputación por tipo de RN-019), participa de los **filtros de sección** (RF-VM-006) y entra en la **evaluación de límites** pasiva y activa (RN-022). En **`/reportes`** entra **solo** en las cards que lo habilitan (opt-in por card, apagado por defecto, RF-REP-017); en el **dashboard** no entra nunca. La fila simulada **no expone acciones de movimiento ni card de detalle** y no puede ser origen de un calculado. Ni crear, ni extender ni eliminar una simulación se registran en el historial, y la eliminación **no es deshacible**: es la excepción a RN-027, documentada en RF-SIM-004. |
...
|---|---|
| Reportes: otros tipos (torta, barras de comparación, etc.) | Los tipos de reporte que **sí** entran son ingresos/gastos y apilado por categoría de gastos (RF-REP-001), la grilla anual de gastos Únicos día × mes (RF-REP-010), el gantt anual de gastos en Cuotas (RF-REP-011), las líneas de Inflación vs Ingresos (RF-REP-012) y el Detalle histórico de gastos fijos (RF-REP-013). Sumar nuevos tipos de reporte queda fuera de alcance: es una mini-fase futura que requiere definición de UX y no es bloqueante |
...
| Movimiento simulado | Movimiento **derivado al vuelo** que una simulación de categoría aporta a la sección Únicos de un mes de **su tramo** (**mes en curso incluido**). Es un **remanente**: lo que falta para llegar a lo proyectado, descontando los únicos reales del mes. No existe como fila y no es editable ni accionable. Se ve **siempre** en `/mes` y, **opt-in por card**, en `/reportes` (RF-REP-017). Su dirección y magnitud salen del cálculo (RN-028). Ver RF-SIM-003. |
...
| Simulación de categoría | Configuración del usuario (usuario + categoría + **tramo propio** `startMonth`/`endMonth`, RN-028/029) que proyecta, a lo largo de ese tramo (que nunca alcanza meses pasados), el total de **movimientos únicos** de esa categoría, por regresión lineal sobre los 12 meses anteriores. Se crea (de a una o varias categorías), se extiende hacia adelante por su `endMonth` (1, 3, 6 o 12 meses) y se elimina, las tres desde el filtro de la sección Únicos de `/mes`; el `startMonth` es inmutable y eliminarla es definitivo. Ver módulo 3.15 (RF-SIM-001..005), RF-REP-017, RN-028/029. |
