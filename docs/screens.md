...
- **Proyección de fijos (solo `income-expense` de `/reportes`, RF-REP-015)** — toggle on/off (default off) que **extiende las series a los meses futuros** proyectando solo los fijos (cuotas y únicos no se proyectan); el tramo proyectado se diferencia visualmente del real, con **horizonte ilimitado**. La regla de cálculo es del backend y el detalle visual lo define `control-design`.
...
- **Tipo Ingresos vs Gastos (`income-expense`):** por cada mes, el total de ingresos y el total de gastos (cada total suma únicos + fijos activos + cuotas del mes, igual que RF-VM-002), **restringido a las categorías seleccionadas**. Es **Total-only**: solo las dos series agregadas, sin sub-vista por categoría ni toggle de representación. En `/reportes` puede **extender las series a futuro** con la proyección de fijos (RF-REP-015), diferenciada visualmente del tramo real.
...
- **Activar/desactivar la proyección de fijos** (solo `income-expense` de `/reportes`, RF-REP-015) — el toggle extiende o recorta el tramo futuro proyectado sin tocar año ni filtros; se persiste por card (clave `reports`, campo `projectFixed`). Si una serie está oculta (leyenda o dirección), su proyección también desaparece.
