...
- **Navegar el año de una card** y **filtrar sus categorías** — embebidos en cada card (widget autónomo, pantalla 8); cada cambio se persiste. Son **independientes por card**: no hay control de año ni filtro compartidos. En **"Detalle histórico de gastos fijos"** el filtro por categoría **no existe**: lo reemplaza una **selección por gasto fijo individual** (qué líneas se dibujan), y esa card además ofrece un **selector de modo** (montos / variación % / variación % ajustada por inflación), todo persistido por card (RF-REP-013).
