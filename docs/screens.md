...
- **Simular categorías** (RF-SIM-001) — desde el disparador de filtro de la sección **Únicos**, la acción "Simular categoría" ofrece las **categorías activas** del usuario y crea simulaciones sobre **una o varias** en una sola confirmación. Una categoría **sin 3 meses con únicos** en los 12 meses anteriores, o **ya simulada**, se ofrece **deshabilitada con el motivo visible** (nunca oculta). Si alguna de las elegidas falla, las demás **se crean igual** y la selección queda abierta con el motivo por categoría, para reintentar solo esas (RF-SIM-001). Al confirmar, los meses del horizonte —**desde el mes en curso**— pasan a mostrar su movimiento simulado. La simulación **no se edita**: no se ajusta monto ni dirección. La pantalla **expone el horizonte vigente** (hasta qué mes alcanza la proyección, RF-SIM-002) en las dos superficies donde el usuario lo necesita: al elegir la categoría y en el listado de simulaciones activas.
- **Eliminar una simulación** (RF-SIM-004) — el mismo disparador lista las **simulaciones** del usuario; al eliminar una y confirmar, sus movimientos simulados dejan de derivarse en todos los meses del horizonte y los totales se recalculan. La eliminación es **definitiva**: no queda registrada en el historial y no se puede deshacer; recuperarla es volver a crear la simulación.
...
- **Mes del horizonte con simulación activa (RF-SIM-003):** la sección Únicos lista, junto a los movimientos reales del mes, los **simulados** de cada simulación que alcanza ese mes —**el mes en curso incluido**—, diferenciados visualmente y sumando a subtotal, contador y totales, que **señalan que incluyen simulados** mientras haya alguno a la vista (RF-SIM-003). Un mes **fuera** del horizonte, o sin simulaciones activas, se ve exactamente igual que sin la feature. Un mes cuyo **remanente** redondea a **0** o se cancela por cambio de signo (RF-SIM-002) no lleva fila simulada de esa simulación.
- **Simulación pausada (RF-SIM-002):** una simulación cuya categoría queda por debajo de los **3 meses con únicos** sigue existiendo pero **no deriva movimientos**. La pantalla lo explica en las dos superficies donde el usuario puede preguntárselo: el listado de simulaciones del disparador de filtro (con el motivo por simulación) y el listado de Únicos del mes del horizonte donde esperaba ver la fila. Vuelve a proyectar sola si la categoría recupera datos.
...
- **Toggle de movimientos simulados** (RF-REP-017) — solo en las cards **"Ingresos vs Gastos"** y **"Gastos por categoría"**: un control que habilita los **movimientos simulados** (RF-SIM-003) en el tramo del año que la card muestra alcanzado por el horizonte, **desde el mes en curso**. **Apagado por defecto** y **persistido por card**. Se ofrece **deshabilitado con el motivo visible** —nunca oculto— cuando el usuario no tiene ninguna simulación o cuando el año de la card **ya pasó** (año anterior al año en curso); el **año en curso siempre queda habilitado**, incluso en diciembre. Deshabilitarlo no borra el valor persistido. Las otras cuatro cards no lo exponen.
...
- **Card con movimientos simulados habilitados (RF-REP-017):** los meses del año en curso **desde el mes en curso** incorporan los simulados, distinguidos dentro del gráfico; los meses pasados no cambian. El **mes en curso queda mixto** (real a la fecha + remanente, RF-SIM-002). Sin simulaciones activas, con todas pausadas o en un año ya pasado, la card se ve igual que con el toggle apagado, sin error.
