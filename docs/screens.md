...
- **Lista de movimientos agrupada por tipo** en **tres secciones colapsables** (acordeón) rotuladas — **Únicos**, **Fijos**, **Cuotas** (RF-VM-001, RF-VM-005). El orden de las secciones es el default salvo que el usuario lo haya reordenado (RF-VM-005). Dentro de cada sección, los movimientos se ordenan por **monto descendente** (el monto más alto primero, por magnitud, sin distinguir gasto de ingreso), con el desempate por sección que define el contrato de `GET /movements` (ver `data-model.md`). En **Únicos**, el usuario puede alternar el orden entre **por monto** (default) y **por fecha** (más reciente primero) desde el control de orden de su cabecera (RF-VM-001); el orden se persiste. En **Fijos** y **Cuotas** el orden de ítems no es alterable.
  ...
  - **Movimientos simulados (RF-SIM-003).** En un **mes futuro** dentro del horizonte de una simulación activa (RF-SIM-002), la sección **Únicos** suma **un movimiento simulado por simulación**, derivado al vuelo y **claramente distinguible** de los reales. Muestra la **categoría simulada** como identidad, su **tipo** (derivado del signo del cálculo) y su **magnitud** en la moneda default; **no** tiene fecha/hora, método de pago, descripción propia ni cotización propia. **Suma** al subtotal y al contador de la sección y a los totales del mes, y participa de los filtros de la sección. Su fila **no** expone kebab (⋮) ni abre la card de detalle. En el orden por monto entra por magnitud; en el orden por fecha va **al final**. El mes en curso y los pasados **nunca** llevan movimientos simulados. El detalle visual lo define `control-design` (`docs/design.md`).
