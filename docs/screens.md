...
**RF relacionados:** RF-VM-001, RF-VM-002, RF-VM-003, RF-VM-004, RF-VM-005, RF-VM-006, RF-VM-007, RF-VM-008, RF-MF-005, RF-MF-006, RF-MF-007, RF-CUR-005, RF-PM-006, RF-LIM-005, RF-SIM-001, RF-SIM-002, RF-SIM-003, RF-SIM-004
...
- **Lista de movimientos agrupada por tipo** en **tres secciones colapsables** (acordeón) rotuladas — **Únicos**, **Fijos**, **Cuotas** (RF-VM-001, RF-VM-005). El orden de las secciones es el default salvo que el usuario lo haya reordenado (RF-VM-005). Dentro de cada sección, los movimientos se ordenan por **monto descendente** (el monto más alto primero, por magnitud, sin distinguir gasto de ingreso), con el desempate por sección que define el contrato de `GET /movements` (ver `data-model.md`). En **Únicos**, el usuario puede alternar el orden entre **por monto** (default) y **por fecha** (más reciente primero) desde el control de orden de su cabecera (RF-VM-001); el orden se persiste. En **Fijos** y **Cuotas** el orden de ítems no es alterable.
  ...
  - **Movimientos simulados (RF-SIM-003).** En un mes dentro del **tramo** de una simulación activa —**el mes en curso incluido** (RF-SIM-002)—, la sección **Únicos** suma **un movimiento simulado por simulación**, derivado al vuelo y **claramente distinguible** de los reales. Muestra la **categoría simulada** como identidad, su **tipo** (derivado del signo del cálculo) y su **magnitud** en la moneda default; **no** tiene fecha/hora, método de pago, descripción propia ni cotización propia. **Suma** al subtotal y al contador de la sección y a los totales del mes, y participa de los filtros de la sección. Su fila **no** expone kebab (⋮) ni abre la card de detalle. En el orden por monto entra por magnitud; en el orden por fecha va **al final**. Lo que muestra es el **remanente** del mes (lo que falta para llegar a lo proyectado, descontando los únicos reales ya cargados), así que en el **mes en curso** convive con los movimientos reales sin duplicarlos. Los meses **pasados nunca** llevan movimientos simulados. El detalle visual lo define `control-design` (`docs/design.md`).
...
- **Modal de alcance de la anulación** (RF-MF-005), superpuesto a la pantalla, disparado desde la acción de anular del kebab de un ítem **fijo**. Contiene:
  - El **nombre del fijo** sobre el que se opera.
  - Dos **opciones de alcance** excluyentes: **este mes** (con el mes visualizado nombrado) — **preseleccionada** — y **un rango de meses**.
  - Con el rango elegido, dos **selectores de mes/año** (desde y hasta, ambos inclusive) acotados a los límites de RF-MF-005 (arranque y último mes de aparición del fijo, largo máximo de 24 meses); admiten meses pasados. El modal indica **cuántas apariciones** del fijo abarca el rango elegido, que puede ser menor a la cantidad de meses cuando la frecuencia no es mensual (RF-MF-006).
  - **Confirmar** y **cancelar**. El **verbo** de todo el modal —anular o des-anular— lo fija el estado del mes visualizado, y aplica a las dos opciones por igual.
...
- **Eliminar** un ítem — dispara el flujo de eliminación correspondiente al tipo (RF-MU-003 único, RF-MF-004 fijo, RF-MC-002 grupo de cuotas), con su confirmación específica. Al eliminar un **fijo**, la confirmación **no ofrece opciones** y la eliminación aplica desde el **mes activo (el mes visualizado) inclusive en adelante**, mismo pivote que la edición (RF-MF-004 / RN-005). Editar y eliminar quedan registrados en el historial y se pueden **deshacer** desde `/historial` (§11, RF-HIST-001/003) o desde el propio **toast de éxito**, que ofrece "Deshacer" sin confirmación (RF-HIST-007); ni el formulario de edición ni la confirmación de borrado exponen control o aviso alguno al respecto.
- **Anular / Des-anular** (solo en ítems **fijos**, RF-MF-005) — en el menú de acciones del ítem fijo, además de Editar y Eliminar, la acción **"Anular este mes"** (**"Des-anular este mes"** si el fijo ya está anulado en el mes visualizado) abre el **modal de alcance de la anulación** (ver abajo), que aplica la operación a **un mes** o a **un rango de meses**. El ítem anulado **se sigue mostrando** (no desaparece), deja de sumar a los totales de su mes y tiene diferenciación visual. Los movimientos **únicos** y las **cuotas** tienen su propia acción de anulación, directa y sin modal. La anulación no afecta los meses de afuera del alcance elegido, a diferencia de Eliminar (RF-MF-005 / RN-016).
...
- **Simular categorías** (RF-SIM-001) — desde el disparador de filtro de la sección **Únicos**, la acción "Simular categoría" abre un selector que lista **solo las categorías simulables** del catálogo activo del usuario: quedan **ocultas** —no deshabilitadas— las que tienen **menos de 3 meses con únicos** en los 12 meses anteriores y las que **ya están simuladas**. Al pie de la lista, una **nota de elegibilidad** explica ese criterio, para que la ausencia de una categoría sea explicable sin mostrarla. El **universo se congela al abrir** el selector: ninguna categoría desaparece mientras el usuario elige, aunque los datos cambien detrás. Si no queda ninguna categoría simulable, un **estado vacío** explica la causa entre **cuatro** posibles: el usuario no tiene categorías activas; ninguna llega a 3 meses con datos; ya están simuladas todas las que se pueden simular; o una combinación de las dos anteriores (a las que tienen datos ya se les simuló, a las demás les faltan meses). El usuario elige **una o varias** y confirma en una sola operación: todas quedan **ancladas al mes visualizado** (RN-028) — si el mes visualizado es anterior al mes en curso, el tramo de cada una arranca igual desde el mes en curso, sin revivir meses pasados. Si alguna de las elegidas falla, las demás **se crean igual** y la selección queda abierta con el motivo por categoría, para reintentar solo esas (RF-SIM-001). Al confirmar, los meses del **tramo de cada simulación** pasan a mostrar su movimiento simulado. El único eje editable de una simulación es el **fin del tramo** (RF-SIM-005): el monto y la dirección no se ajustan a mano —salen del cálculo— y el `startMonth` es inmutable. El listado de simulaciones activas del mismo disparador muestra, **por simulación**, hasta qué mes alcanza su propio tramo (RF-SIM-002) — no hay un único horizonte compartido.
- **Eliminar una simulación** (RF-SIM-004) — el mismo disparador lista las **simulaciones** del usuario; al eliminar una y confirmar, sus movimientos simulados dejan de derivarse en todos los meses de su tramo y los totales se recalculan. La eliminación es **definitiva**: no queda registrada en el historial y no se puede deshacer; recuperarla es volver a crear la simulación.
- **Extender el tramo de una simulación** (RF-SIM-005) — en la fila de una simulación dentro del mismo disparador, la acción **"Extender"** aparece **solo cuando el mes visualizado en `/mes` es exactamente el `endMonth` de esa simulación**; en cualquier otro mes esa fila no la ofrece. Al elegirla ofrece cuatro incrementos —**1, 3, 6 o 12 meses**— que se suman al `endMonth` vigente; el `startMonth` nunca se toca. No pide confirmación (es aditiva, no destructiva): un toast confirma el éxito y el popover **no se cierra**. Al extender, ese mes deja de ser el `endMonth`, así que la acción no se puede repetir desde el mismo lugar hasta navegar al nuevo último mes — por eso no hay tope de extensión. Una simulación **pausada**, o una cuyo `endMonth` ya pasó, se extiende igual, sin rama especial ni marca de "vencida" o "terminada".
- **Ver los límites que observan la vista** (RF-LIM-005) — el ícono informativo junto al rótulo del mes abre el popover de solo lectura con el listado. Es informativo y **sin salida**: no edita, no activa/desactiva y no lleva a `/configuracion/limites` (ahí se gestionan, RF-LIM-002); sus únicas interacciones son abrir y cerrar.
...
- **Mes dentro del tramo de una simulación activa (RF-SIM-003):** la sección Únicos lista, junto a los movimientos reales del mes, los **simulados** de cada simulación cuyo tramo alcanza ese mes —**el mes en curso incluido**—, diferenciados visualmente y sumando a subtotal, contador y totales, que **señalan que incluyen simulados** mientras haya alguno a la vista (RF-SIM-003). Un mes **fuera del tramo de todas** las simulaciones activas, o sin simulaciones activas, se ve exactamente igual que sin la feature. Un mes cuyo **remanente** redondea a **0** o se cancela por cambio de signo (RF-SIM-002) no lleva fila simulada de esa simulación.
- **Simulación pausada (RF-SIM-002):** una simulación cuya categoría queda por debajo de los **3 meses con únicos** sigue existiendo pero **no deriva movimientos**. La pantalla lo explica en las dos superficies donde el usuario puede preguntárselo: el listado de simulaciones del disparador de filtro (con el motivo por simulación) y el listado de Únicos del mes de su tramo donde esperaba ver la fila. Vuelve a proyectar sola si la categoría recupera datos.
...
- **Guardar / Confirmar** — valida y persiste. Al guardar con éxito, el modal se cierra y aparece un toast de confirmación: en **creación**, con la acción "Ir a ver" (RF-MU-001, RF-MF-001, RF-MC-001); en **edición**, con la acción "Deshacer" (RF-HIST-007). Si la proyección del movimiento cruzaría uno o más límites **activos** (RF-LIM-004), antes de persistir se intercala un **diálogo de aviso** no bloqueante (ver Estados); aplica a los cuatro forms (único / fijo / cuota / calculado).
...
**RF relacionados:** RF-REP-001, RF-REP-002, RF-REP-003, RF-REP-004, RF-REP-010, RF-REP-011, RF-REP-012, RF-REP-013, RF-REP-017, RF-LIM-005, RF-NAV-001
...
- **Popover informativo de límites por card** (RF-LIM-005) — junto al título de cada card, un **ícono informativo** abre un popover de **solo lectura** con los límites que observan esa card. Misma anatomía y comportamiento que el popover de `/mes` (pantalla 4), con tres diferencias: el caption es **"Límites de este reporte"**; solo aparece el grupo **"Marcan un dato"** (los reportes no admiten límites activos, así que nunca hay grupo "Avisan al guardar"); y la superficie es el **tipo de reporte, no la card**, así que dos cards del mismo tipo listan lo mismo y el listado **no cambia** con el año, el filtro ni la moneda de la card.
- **Toggle de movimientos simulados** (RF-REP-017) — solo en las cards **"Ingresos vs Gastos"** y **"Gastos por categoría"**: un control que habilita los **movimientos simulados** (RF-SIM-003) en el tramo del año que la card muestra alcanzado por la **unión de tramos** de las simulaciones del usuario, que arranca **como mínimo desde el mes en curso**. **Apagado por defecto** y **persistido por card**. Se ofrece **deshabilitado con el motivo visible** —nunca oculto— cuando el usuario no tiene ninguna simulación, o cuando el año de la card **ya pasó** (año anterior al año en curso) —en ese caso el motivo explica que las simulaciones **no alcanzan meses pasados**—; el **año en curso siempre queda habilitado**, incluso en diciembre. Deshabilitarlo no borra el valor persistido. Las otras cuatro cards no lo exponen.
...
- **Navegar el año de una card** y **filtrar sus categorías** — embebidos en cada card (widget autónomo, pantalla 8); cada cambio se persiste. Son **independientes por card**: no hay control de año ni filtro compartidos. En **"Detalle histórico de gastos fijos"** no hay navegación de año ni filtro por categoría: el filtro lo reemplaza una **selección por gasto fijo individual** (qué líneas se dibujan) y el año lo reemplaza un **select de rango** de 8 opciones (3/6/9 meses y 1/2/3/4/5 años, default 3 años) que **siempre termina en el mes en curso**; esa card además ofrece un **selector de modo** (montos / variación % / variación % ajustada por inflación), todo persistido por card (RF-REP-013). Los fijos que no llegan a **2 apariciones graficables** en el rango quedan fuera del gráfico y de la leyenda, pero la card los **sigue exponiendo aparte**, con su mes de inicio.
...
- **Habilitar / deshabilitar los movimientos simulados de una card** (RF-REP-017) — solo en Ingresos vs Gastos y Gastos por categoría. Habilitado, los meses del año en curso desde el **mes en curso** en adelante suman los movimientos simulados de las simulaciones activas —en Gastos por categoría, **solo los que resultan gasto**— distinguidos dentro del gráfico y sin aviso textual; los filtros de la card los alcanzan como a cualquier **único** (tipo, dirección derivada y categoría simulada). El cambio se **persiste por card** y no afecta a las demás.
- **Ver los límites que observan una card** (RF-LIM-005) — el ícono informativo junto al título de la card abre el popover de solo lectura con el listado. Es informativo y **sin salida**: no edita, no activa/desactiva y no lleva a `/configuracion/limites`; sus únicas interacciones son abrir y cerrar.
...
- **Card con movimientos simulados habilitados (RF-REP-017):** los meses del año en curso que caen dentro de la **unión de tramos** de las simulaciones activas (como mínimo desde el mes en curso) incorporan los simulados, distinguidos dentro del gráfico; los meses pasados no cambian. El **mes en curso queda mixto** (real a la fecha + remanente, RF-SIM-002). Sin simulaciones activas, con todas pausadas o en un año ya pasado, la card se ve igual que con el toggle apagado, sin error.
...
- **Tramo futuro con simulados** (RF-REP-017, solo `income-expense` y `by-category` con el toggle habilitado): los meses del año en curso que caen dentro del **tramo de alguna simulación activa** suman, además de lo que proyectan fijos y cuotas (RN-006), los **movimientos simulados** —en `by-category`, solo los que resultan gasto—, distinguidos dentro del gráfico y **sin aviso textual** de composición. El mes en curso y los pasados no cambian.
...
- **Navegar de año** — recalcula el recuadro para el año seleccionado, dentro de los límites: hacia atrás el control ‹ se deshabilita antes del **primer año con CUALQUIER movimiento del usuario** (`earliestYear`, no afectado por el filtro); hacia adelante los años futuros quedan bloqueados (máximo navegable = año en curso). Las cards con **universo de datos propio** fijan sus propios topes en su RF (`unique-grid`, RF-REP-010). **`fixed-evolution` no navega**: en vez de stepper de año tiene un **select de rango** de meses corridos (3/6/9 meses y 1/2/3/4/5 años) cuyo **borde derecho es siempre el mes en curso** —nunca grafica meses futuros— y cuyo **borde izquierdo se recorta** contra el primer mes con gasto fijo (RF-REP-013).
...
**RF relacionados:** RF-HIST-001, RF-HIST-002, RF-HIST-003, RF-HIST-004, RF-HIST-005, RF-HIST-006, RF-NAV-001, RF-SIM-004
