...
**La base de datos local de desarrollo es descartable.** Sus datos no tienen valor. Al correr QA (o cualquier prueba), el orquestador **crea, modifica y borra** libremente lo que necesite —categorías, movimientos, métodos de pago, calculados, simulaciones, etc.— **sin pedir permiso** y **sin obligación de revertir** la data de prueba. La data no se trata como preciosa. Esto aplica **solo a la base local de desarrollo**, no a datos de producción.

**Interactuar es el guion, no una excepción:** en **localhost**, el orquestador **usa la app y envía formularios sin pedir confirmación por acción** — crear, guardar y **borrar** registros de prueba (movimientos, categorías, métodos, límites, duplicados) es exactamente lo que el QA pide. Pedir permiso antes de cada submit invalida el recorrido.

**Límites (reglas de seguridad):** el orquestador **no crea cuentas de usuario, no ingresa credenciales/contraseñas ni realiza el login** (incluido Google OAuth). Si una prueba requiere una sesión autenticada, la autenticación la resuelve el usuario. El **alta, la edición y el borrado** de datos los maneja el orquestador sin fricción.

**El borrado está dentro del guion**, como corolario de lo anterior: si la data no es preciosa, la irreversibilidad de un borrado no cuesta nada. El orquestador los **ejecuta él mismo, sin pedir confirmación**, igual que el resto del guion —crear, editar, **anular/des-anular**, navegar—. **Anular no es borrar:** es un caso reversible más y se ejercita con normalidad. Vale **solo contra la base local**; contra **producción** no se borra nada.
...
Superficies (recorrelas todas): Login (Google); /mes (movimientos por sección Únicos/Fijos/Cuotas, navegación de meses, menú de acciones editar/eliminar/anular por ítem); banda "Simulación" del popover de filtro de la sección Únicos (botón de alta, lista de simulaciones con su acción de eliminar y el tramo propio de cada una); modal selector de categoría a simular (solo categorías simulables, candidatas inelegibles ocultas —no deshabilitadas—, nota de elegibilidad al pie, los cuatro estados vacíos por causa); fila de movimiento simulado en un mes futuro (mezclada con los únicos reales, sin kebab ni card de detalle); alta/edición de movimiento (modal con tipos Único/Fijo/Cuota/Calculado; campos monto, descripción, categoría, fecha, y en "Más opciones" moneda+cotización y método de pago + débito automático); Dashboard; /reportes (crear/configurar 6 tipos de card, filtros por categoría, orden, año, refrescar); card "Detalle histórico de gastos fijos" (leyenda con un ítem por fijo como control de selección en lugar del filtro por categoría, con 20-30 ítems; modos Montos/Variación, que cambian la unidad del eje Y entre moneda y puntos porcentuales; chip "Ajustada por inflación" deshabilitado con motivo en Montos y activo en Variación; calculados como líneas punteadas propias; meses sin dato —frecuencia, anulado, fuera de vigencia— que no se grafican en cero y explican el motivo por tooltip; select de rango con 8 opciones —3/6/9 meses y 1 a 5 años— que estira el eje hacia atrás desde el mes en curso, sin graficar nunca meses futuros; lista de fijos sin evolución —los de menos de 2 apariciones en el rango, con su mes de inicio—); /historial (entradas de edición/eliminación, deshacer simple y en cadena); /configuracion (solapas General [moneda, tema] y Límites); Categorías (alta/edición/borrado, color, soft delete); Métodos de pago (alta/edición/borrado, tipo crédito/débito/efectivo, campos según tipo); superficie de captura (con el viewport por debajo de 600px en cualquiera de los dos ejes: tabs Único/Fijo/Cuotas, todos los campos incluido "Más opciones", aviso de límite, confirmación al guardar sin navegación, identidad de la sesión y cierre de sesión, y que nada más de la app sea alcanzable); acceso en régimen de captura (login sin registro, y que cruzar la frontera de 600px en los dos sentidos conmute de superficie sin recargar ni parpadear).
