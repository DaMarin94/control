...
- **Modal read-only sobre `ModalShell`** (variante `dialog`, con `closeOnScrimClick` → cierra por ✕/Esc/scrim). **No dispara fetch:** renderiza el `MovementItem` que la lista ya tiene; sin estados de carga/vacío/error propios. Su único control de acción es **"Editar"**, que cierra la card y abre el modal de edición del tipo correspondiente (mismo flujo que el kebab).
