...
- **`KebabMenu` (`components/ui/kebab-menu.tsx`)** — menú de tres puntos para las acciones de fila **editar / eliminar** en listas. Es el **componente estándar para esas acciones**: toda lista nueva que las necesite lo usa, en lugar de botones inline en la fila. Se renderiza **por portal a `document.body` con posición `fixed`** (coordenadas tomadas del `getBoundingClientRect()` del trigger) por DOS razones estructurales: (a) las tarjetas de lista tienen `overflow-hidden`, que recortaría un menú `absolute`; (b) el portal lo saca del ancestro transformado (ver "Modales y diálogos: portal a `body`"). API: `ariaLabel`, `items` (`{ label, icon?, onSelect, danger? }`), `className`. El ítem `danger: true` se pinta en `text-expense-ink`. El trigger se mantiene visible mientras el menú está abierto; el menú se cierra en select / click afuera / Escape / scroll / resize (no recalcula posición en movimiento: cierra limpio).
  - **Gotcha — la altura del panel se deriva de la cantidad de ítems, no de una constante fija.** La decisión de abrir hacia abajo o hacia arriba se calcula con la **altura real** del menú (`items.length × alto de ítem + padding`). El **contrato del componente es que el panel entra entero en el viewport con cualquier cantidad de ítems**: es compartido por todas las listas, así que agregar un ítem no puede volver a apoyarse en una estimación fija de alto (con un menú más largo, el flip se decide tarde y el panel queda cortado abajo).

### Primitivas de formulario — `min-w-0` en toda la cadena (gotcha)

`Input` (`components/ui/input.tsx`) y `Select` (`components/ui/select.tsx`) llevan **`min-w-0` en toda su cadena de contenedores** —el wrapper externo (que además es `w-full`), el `div.relative` del `Select` y el control mismo—. Los hijos de `grid`/`flex` tienen `min-width: auto`, o sea **no pueden achicarse por debajo del min-content de su contenido**, y `w-full` no lo evita. WebKit (iOS Safari) le asigna a los controles nativos —`input[type=date|time|month|number]` y `<select>`, cuyo mínimo es el del option más largo— un ancho intrínseco **mayor** que Blink; sin `min-w-0` el track del grid crece por encima del ancho del viewport y el formulario gana scroll horizontal en pantallas angostas. (Referencia: en Chrome el min-content es 173px para `type=date` y 109px para `type=time`; en WebKit es mayor.)

- **Consecuencia operativa:** al armar una fila de dos columnas (`grid-cols-2`) con campos, **los grid items también necesitan `min-w-0`** — no alcanza con que lo tengan las primitivas. Aplicado en las filas de dos columnas de la superficie de captura: `capture-transaction-form.tsx` (Fecha + Hora), `capture-installment-form.tsx` (Cant. de cuotas + Mes de inicio) y `capture-more-options.tsx` (Moneda + Cotización).
...
- **Cierre por clic en el scrim — opt-in (`closeOnScrimClick`, default `false`).** Por defecto los modales **no** cierran al clickear el fondo (son de decisión: transaction-modal, confirmaciones). Un modal auxiliar/read-only que quiera el cierre tipo popover (✕ + Esc + clic afuera) pasa `closeOnScrimClick`; hoy solo lo usa `MovementDetailCard` (RF-VM-007). **Ver el gotcha de burbujeo del scrim abajo** — es obligatorio cuando el modal se monta dentro de un trigger clickeable.
...
### Modal portado con `closeOnScrimClick` dentro de un trigger clickeable — `stopPropagation` en el scrim (gotcha)
...
## Gate de arranque del backend (RF-APP-005)

Superficie a pantalla completa que tapa la app mientras el backend está inalcanzable. Regla funcional en `requirements.md`, RF-APP-005; tratamiento visual en `docs/design.md`. Motivo estructural de la espera larga (cold start del web service) en `docs/technical.md`, §Deploy (Render). Piezas: `src/lib/backend-gate.ts` (señalización), `src/hooks/use-backend-gate.ts`, `src/components/backend-gate/`.

- **Criterio de la sonda (`GET /health`): cualquier respuesta HTTP cuenta como backend vivo, incluso un `500`.** Solo el fetch **rechazado** (sin respuesta) significa dormido. Un backend que responde mal está despierto, y taparlo escondería el error real. Sin `NEXT_PUBLIC_API_URL` la sonda da por vivo: una config faltante no puede pintar un gate eterno.
- **La sonda no lleva timeout propio.** Con el servicio suspendido el request queda colgado hasta que levanta, así que un pedido pendiente alcanza. El **backoff** (1s → 10s, luego estable) existe solo para el **fallo rápido** (backend apagado, `ECONNREFUSED`), para no martillar.
- **Señalización por pub/sub a nivel de módulo** en `src/lib/backend-gate.ts` (`notifyBackendUnreachable` / `notifyBackendRecovered` + sus `subscribe`), **mismo patrón que `emitToast`**: punto único, sin repetir chequeos por hook. El `ApiError` con `statusCode` **503** (error de red, emitido por `src/lib/api.ts`) dispara la señal de inalcanzable desde el **manejo centralizado de errores** de `src/lib/react-query.tsx`, donde ya vive el manejo del `401`.
- **Recuperación de datos — por qué nunca se re-ejecuta una mutación.** Al recuperarse el backend se llama `refetchQueries({ type: "active", predicate: query => query.state.status === "error" })`. `refetchQueries` opera **solo sobre la `QueryCache`**; las mutations viven en la `MutationCache` y son **inalcanzables desde esa API**, así que la garantía de no reejecutar una escritura fallida (que podría **duplicar un gasto**) es **estructural, no un filtro**. El `predicate` acota el rescate a lo que murió, para no disparar una tormenta contra un backend recién arrancado y frío. Descartados: `invalidateQueries()` sin filtro (marca stale **todo**) y `resetQueries` (vaciaría data sana y la mandaría de vuelta a skeleton).
- **La señal de recuperación se emite antes de la animación de salida**, no al desmontar: si saliera después, el usuario vería el cartel de error un instante antes del refetch.
- **Montaje: tercer hermano en `src/app/layout.tsx`, fuera de `capture:hidden` y fuera de `CaptureSurfaceRoot`** — una sola superficie sirve a los dos regímenes. **No lleva variante `capture:` de visibilidad**: ocultarlo en un régimen lo dejaría sin gate. `z-index` **200**, por encima de modales (40/50) y toasts (90).
- **El fondo se vuelve inerte marcando los hermanos hacia arriba hasta `<body>`**, no marcando los hijos de `<body>`: el gate vive dentro del árbol de providers, que puede introducir nodos DOM intermedios. Así también quedan inertes los **toasts**, que portan a `body`.
- **Los umbrales de `capture` / `short` (600 / 480) se escriben como media queries crudas en `globals.css`**, no como variantes Tailwind: el alto del botón lo definen **los dos a la vez** y el orden de variantes de Tailwind no es determinístico. Precedente en el mismo archivo: `--toast-inset-bottom`.
...
- **Props como discriminated union por `mode`** — el modal se comporta distinto según el modo y TypeScript fuerza el shape correcto en cada caso:
  ...
  - **`"edit-single"` / `"edit-fixed"` / `"edit-installment"`** — sin tabs; el form del tipo, precargado con un `Transaction` / `Recurring` / grupo de cuotas (RF-MU-002, RF-MF-003, RF-MC-003).
  - **`"duplicate-single"` / `"duplicate-fixed"` / `"duplicate-installment"`** — sin tabs; el form del tipo en **modo creación**, alimentado por la prop `*Prefill` (RF-VM-008; ver §Duplicar).
  - **`"create-calculated"` / `"edit-calculated"`** — sin tabs; `CalculatedForm` (ver §Movimientos calculados).
- **`"create"` es el único modo con tabs.** Ni edición, ni duplicar, ni calculado los muestran: el tipo de un movimiento no se elige fuera del alta en blanco (RF-CM-001).
...
- **Los effects de pre-carga de la cotización (`referenceRate` / `lastExchangeRate`) no pisan un valor precargado** — se comportan igual que en edición cuando el form corre con `prefill` (ver §Duplicar). Es lo que sostiene que la cotización duplicada sea la del original y no la de referencia del mes (RF-VM-008).
...
### Duplicar — prefill desacoplado de `isEditing` (RF-VM-008)

Aplica a los **tres** forms (`transaction-form`, `recurring-form`, `installment-form`). Regla funcional en `docs/requirements.md`, RF-VM-008; acá solo lo estructural.

- **`isEditing` gobierna dos cosas a la vez** en estos forms: "precargar valores" y "PATCH vs POST" (`isEditing = entidad !== null`). Duplicar necesita **lo primero sin lo segundo**, así que la pre-carga entra por una prop aparte, **`prefill`**, que llena los `defaultValues` **sin** activar `isEditing`: el submit sigue siendo un **POST**. Un cambio futuro no debe volver a colapsar ambas cosas en `isEditing`.
- **Consecuencia a respetar en los effects:** todo effect que pre-cargue un campo desde una fuente externa (hoy la cotización, `referenceRate` / `lastExchangeRate`, y la moneda default) tiene que tratar el modo prefill **como si fuera edición** y no pisar el valor copiado. Por eso la nota del campo lee **"Cotización modificada"**: el valor viene del original, no de la referencia del mes.
- **Los mappers de prefill son propios: `movementItemTo*Prefill`, separados de los `movementItemTo*` de edición.** Los de edición rellenan campos con **dummies válidos solo para el PATCH** (p. ej. `startMonth: getCurrentMonth()`, `createdAt: ""`) porque el contrato de edición no los envía. **Reusarlos para precargar duplicaría esos dummies dentro de un POST**, que sí los manda: el duplicado nacería con el mes de inicio equivocado. No reusar los de edición para prefill.
- `TransactionModal` expone los modos `duplicate-single` / `duplicate-fixed` / `duplicate-installment` (ver la discriminated union por `mode` arriba): sin tabs, título "Duplicar", form del tipo del original.
...
- **`getLocalTodayString`** — fecha local de hoy en `YYYY-MM-DD` con getters locales de `Date` (**nunca `toISOString()`**, que devuelve UTC). Es la **única** fuente del `today` que las superficies mandan al backend; el porqué detallado está en su JSDoc.
- **Helpers de mes:** **`getCurrentMonth`** — mes actual `YYYY-MM` en la zona del navegador; **`formatMonthLabel`** — `YYYY-MM` → rótulo legible (nombre de mes + año); **`formatMonthShort`** — `YYYY-MM` → mes abreviado + año (`"Mar 2024"`), para el arranque y la vigencia del fijo en la card de detalle (RF-MF-007 / RF-VM-007); **`prevMonth` / `nextMonth`** — desplazan un `YYYY-MM` un mes hacia atrás / adelante. Reusarlos para la navegación del mes; no reimplementar aritmética de meses.
...
- **Fila adelgazada (RF-VM-001 / RF-VM-007):** solo lo glanceable — identidad (badge de origen "Único"/"Fijo"/"Cuotas", nombre, categoría), un discriminador en columna propia y el monto convertido en una sola línea. Para fijos muestra la **etiqueta de frecuencia** en minúscula (`FREQUENCY_LABEL[frequency]` de `lib/movements.ts`, rótulos de RF-MF-006); para cuotas la etiqueta **"Cuota X/N"** (de `installment.number` / `installment.total`); para únicos la fecha `DD Mmm`. El **detalle secundario** (método de pago, moneda/valor original, débito automático, hora del único, **arranque y vigencia del fijo**, plan de la cuota, fórmula del calculado) **ya no vive en la fila**: se lee en `MovementDetailCard`. **Null-safety:** `occurredAt` / `timezone` pueden venir `null` (fijos y cuotas) — no pasarlos a `formatDate` / `formatTime` sin chequear.
- **Apertura de la card:** el **cuerpo** de la fila es clickeable (`role="button"`, `Enter`/`Espacio`) y abre `MovementDetailCard`; el **kebab** es un botón hermano que corta la propagación para no abrir la card (ver §Shell de modales, gotcha del scrim). Anatomía y reparto fila/card en `docs/design.md`.

### `MovementDetailCard` (card de detalle, RF-VM-007)

- **Modal read-only sobre `ModalShell`** (variante `dialog`, con `closeOnScrimClick` → cierra por ✕/Esc/scrim). **No dispara fetch:** renderiza el `MovementItem` que la lista ya tiene; sin estados de carga/vacío/error propios. **Read-only pura:** sin footer ni controles de acción (su único control es cerrar) — todas las acciones del movimiento viven en el kebab de la fila (RF-VM-007).
- **Contenido condicional por tipo y por dato presente** (omite las filas que no aplican): método de pago (con la sublínea de crédito `closingDay`/`paymentDay` solo si `type === "CREDIT"`), cross-rate (`currency ≠ default`), débito automático, hora del único, frecuencia + vigencia del fijo (`startMonth`/`endMonth`), plan de la cuota, origen + fórmula del calculado. Anatomía exacta en `docs/design.md`, §Card de detalle de movimiento.
- **`CalculatedChildrenBlock` — derivados del origen (RF-VM-007).** Cuando el ítem es **origen** de calculados (`hasCalculated`), la card lista sus derivados del mes desde `calculatedChildren` (contrato en `docs/data-model.md`). Es el **espejo bidireccional** del bloque "Origen" que un calculado muestra: read-only, **no clickeable** (no navega al calculado). Cada fila muestra nombre + monto. **El monto se muestra con signo derivado del `type`** de cada `CalculatedChild` (`EXPENSE` ⇒ negativo, `INCOME` ⇒ positivo) porque `convertedAmountCents` viene como magnitud (sin signo); no leer el signo de la cifra.
- **Reusa helpers, no reimplementa:** `formatConvertedAmountDisplay` y `FREQUENCY_LABEL` de `lib/movements.ts`; `formatMonthShort` / `formatDate` / `formatTime` / `formatCurrency` de `lib/format.ts`; `descaleOperand` + `buildFormulaExpression` de `lib/formula.ts` para el bloque del calculado (el mismo builder que el preview del `calculated-form`).
...
- **No hay tab "calculado"** en el modal de carga (RF-MCALC-001). El único disparador es la acción **"Crear movimiento calculado"** del kebab de un ítem **fijo, único o cuota** en `/mes` (ícono en `docs/design.md`). `movement-item-row.tsx` la ofrece en cualquier origen **no** calculado (`!movement.calculated`); sigue **sin** aparecer sobre un ítem que ya es calculado (no hay calculado-de-calculado).
...
- **`lib/formula.ts` — builder compartido.** El desescalado del operando (`descaleOperand`) y la construcción de la expresión legible del cálculo (`buildFormulaExpression`) viven en `lib/formula.ts`, no inline en el form. Los reusa `MovementDetailCard` (RF-VM-007) para su bloque "Fórmula": la card y el preview del form muestran la misma expresión sin duplicar la lógica.
...
### Estructura: bloque canónico + flechas portadas al `body`

`PeriodNav` **no es un grid**. Son dos piezas dentro de un wrapper raíz `relative` que ocupa el **100% del ancho de `<main>`** (sin padding propio) — ese wrapper es el sistema de referencia de las flechas:
...
- **Flechas ‹ ›** — el botón visible se monta vía `createPortal` a **`document.body`**, con `position: fixed; top: 50%; translateY(-50%)`, centrado respecto del **viewport real**. **No reservan ancho**: su presencia no angosta ni empuja el bloque; lo flanquean desde afuera sin participar del flujo de `<main>`. Tamaño único **64×64** (glifo 46px), sin variantes por breakpoint. Detalle visual en `docs/design.md`.

**Por qué portal a `document.body`.** `<main>` es `@container` (`container-type: inline-size`), lo que lo vuelve containing block de sus descendientes `position:fixed`/`sticky`: un botón fijado dentro de `<main>` resuelve contra `<main>`, no contra el viewport, y scrollea con la página en vez de quedar clavado. La única salida es sacar el nodo del subárbol de `<main>` — de ahí el portal (ver el gotcha del `@container` en §Navegación global).

**Posicionamiento por medición JS.** Dentro del wrapper raíz quedan dos **anclas `absolute` invisibles** (`aria-hidden`, `pointer-events-none`, sin contenido) que funcionan solo como **sondas**. El hook `useArrowGeometry` mide con `getBoundingClientRect` + `ResizeObserver` (+ resize de ventana): (a) el ancho del wrapper raíz contra el umbral **1288px** para decidir el régimen amplio/compacto, y (b) el `left` resuelto de cada ancla, que da la coordenada horizontal donde se planta el botón portado. El offset horizontal conserva la intención de ~20px de aire respecto del bloque de 1120px, pero se resuelve por medición, no por fórmula CSS. Una medición de ancho 0 (frame inicial sin layout, o jsdom en tests) no se toma como "oculto": conserva el último estado conocido (default visible).

**Sin scroll fantasma con listas cortas** (invariante preexistente): al vivir en `document.body` con `position:fixed`, el botón por definición nunca aporta alto al documento — el alto lo define solo el bloque de contenido.

### Dos regímenes

| Ancho de contenido | Régimen |
| --- | --- |
| ≥1288px | Flechas overlay ‹ › a los costados + título grande en el header. |
| ≤1287px | Flechas `hidden`; el consumidor renderiza el **stepper compacto** en el header. |

El umbral de `PeriodNav` es **1288px** (`--container-arrows`, variant `@arrows:`/`@max-arrows:`), independiente del `941px` (`--breakpoint-wide`, `@wide:`) que rige el colapso de grillas de stat-cards (1↔3 columnas). `1288 = 1120` (cap del bloque) `+ 2×84` (botón de 64 + 20px de aire por lado): es el mínimo de contenido que deja plantar las flechas overlay sin invadir el bloque.

Se mide por **container query sobre `<main>`** (`@arrows:`), **no** contra el viewport: con el sidebar abierto a viewport 1400px el contenido real mide ~1112px → régimen compacto (ver §Navegación global, gotcha del `@container`).
...
- **`month-view-client.tsx` envuelve su contenido en `PeriodNav`** (flechas gigantes ‹ ›) para la navegación prev / next, que cambia `?month=` (con `prevMonth` / `nextMonth`). El rótulo del mes (`formatMonthLabel`) se promueve al **header `.phead`** en ≥1288px (variant `@arrows:`); en ≤1287px el header lleva un **stepper compacto** en lugar de las flechas. Ambos siempre `canGoPrev`/`canGoNext` (en `/mes` no hay topes de navegación). El cap de 1120px lo aporta el propio `PeriodNav` (bloque `max-w-[1120px] mx-auto`); el `px-10` y el padding vertical los agrega `month-view-client.tsx` sobre `children`, y `page.tsx` no lleva `max-w-[1120px] mx-auto` (ver sección Navegación de período).
- **Rótulo de estado del mes — discriminante de TRES estados, separado de `isCurrentMonth`.** El copy (Histórico / Mes en curso / Mes futuro) y su regla están en `docs/screens.md` §4. El valor sale de comparar el mes visualizado con el mes en curso (anterior / igual / posterior) y alimenta **solo presentación**: la sub-línea del header y la prop `statusLabel` del disparador compacto (`month-jump-popover.tsx`), que reciben el **mismo string**. **`isCurrentMonth` es otra cosa y sigue siendo booleano:** es el alcance temporal de los límites (`evaluateItemLimitMark(..., isCurrentMonth)`, RF-LIM-003/004) y el discriminante de los simulados. Tocar `isCurrentMonth` —volverlo ternario, derivar de él el tercer rótulo— cambia qué límites marcan; el rótulo se resuelve con un discriminante **propio y aparte**.
- **Totales del mes** (de `data.totals`).
- **Lista agrupada en secciones colapsables + reordenables Únicos / Fijos / Cuotas** (RF-VM-005). Las **tres secciones se muestran siempre**; una sección vacía muestra cabecera completa + empty inline propio (no hay empty global de la pantalla). Se renderizan con `AccordionSection` / `SortableSection` (ver Sistema de componentes). Cada ítem (`movement-item-row`) es la **fila adelgazada** (identidad + discriminador + monto; ver §`movement-item-row.tsx`); su cuerpo abre `MovementDetailCard` (RF-VM-007) y el kebab expone las acciones **Editar**, **Duplicar** y **Eliminar**, que abren el flujo según el origen: únicos → `TransactionModal` (`edit-single` / `duplicate-single`) / `DeleteTransactionDialog`; fijos → `TransactionModal` (`edit-fixed` / `duplicate-fixed`) / `delete-recurring-dialog`; cuotas → `TransactionModal` (`edit-installment` / `duplicate-installment`) / `delete-installment-dialog`.
...
- **`lib/movements.ts` también nuclea el display por ítem:** `formatConvertedAmountDisplay(movement, defaultCurrency)` (cifra convertida con signo/símbolo, contemplando el signo del calculado que vive en `amountCents`) y `FREQUENCY_LABEL: Record<number, string>` (etiqueta en minúscula por `frequency` 1..12, fuente única para la sublínea de la fila y —capitalizada— la card). Los reusan `MovementItemRow` y `MovementDetailCard` — no reimplementar el formateo ni el mapa de rótulos inline.
...
- **`LimitsInfoPopover({ surface })` es autónomo; la exclusión del Dashboard la impone el LLAMADOR, no la `surface` (RF-LIM-005).** El componente toma solo una `surface`, lee por su cuenta los límites (`useLimits`) y las categorías (`useCategories`), resuelve contra `lib/limits/catalog.ts` y **retorna `null` si no hay ≥1 límite para esa `surface`** (misma señal condicional que el ícono `Info`). La **no aparición en el Dashboard** NO se resuelve dentro del componente vía `surface`: el widget income-expense **efímero** del Dashboard **comparte la misma `surface`** que la card real de `/reportes` (`reporte.ie.*`), así que filtrar por `surface` no los distingue. La asimetría la impone quien lo monta: `report-card.tsx` renderiza el ícono **solo en la rama de card de `/reportes`** (la que trae `onDirectionChange` / chrome de card) y **no** en la rama del widget del Dashboard. Un cambio futuro no debe intentar resolver la exclusión del Dashboard dentro de `LimitsInfoPopover`.
...
- **Mockear hooks de datos: usar los fixtures de `tests/utils/query-result.ts`.** Exporta `mockQuerySuccess(data)`, `mockQueryLoading()` y `mockQueryError(error)` (todos con un segundo parámetro de `overrides` parcial), que arman un `UseQueryResult<T, E>` **completo** — las ~14 propiedades derivadas incluidas (`isLoadingError`, `isRefetchError`, `isPlaceholderData`, `dataUpdatedAt`…). Sirven para mockear cualquier hook construido sobre `useQuery` sin castear un objeto parcial: un parcial casteado a `UseQueryResult` **no compila** (TS2352: le faltan ~17 propiedades) y taparlo con `as unknown as` deja el mock mintiendo sobre el estado de la query. **Regla: reusar estos helpers** — no reimplementar mocks parciales de `useQuery` ni castearlos. `tests/utils/` es la carpeta de utilidades transversales de test, hermana de `tests/setup.ts`, fuera del árbol que espeja `src/`.
- **`renderHook` infiere `TProps` solo de `options.initialProps`**, nunca de la anotación del parámetro del callback. Si `initialProps` se pasa inline sin anotar, TS infiere tipos demasiado angostos a partir de los valores presentes: el literal exacto (`"ARS"`), o directamente `null`/`undefined` cuando ese es el valor inicial de la propiedad. Después cualquier `rerender({...})` con un valor legítimamente distinto falla con TS2322 (`Type '"USD"' is not assignable to type '"ARS"'`, `Type 'string[]' is not assignable to type 'null'`). **Patrón:** declarar `initialProps` como variable con **tipo explícito** —el tipo de dominio real de `src/types/`— antes de pasarla a `renderHook`. No castear en cada `rerender` ni cambiar los valores que el test quiere probar.
