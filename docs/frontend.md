...
- **Dos tarjetas autónomas en `components/charts/report-card.tsx`** — único dueño del código de charting de los dos tipos. Exporta **dos tarjetas independientes**, cada una con su cabecera, gráfico, leyenda, tooltip y estados (skeleton / con datos / vacío / meses futuros / error):
  - **`IncomeExpenseCard`** (`income-expense`, "Ingresos vs Gastos") — **Total-only**: `AreaChart` con las series ingresos y gastos superpuestas. No tiene sub-vista por categoría ni tabs. Su leyenda-filtro togglea series (`hiddenSeries`).
  ...
  - **La leyenda interactiva ES el filtro (RF-REP-002).** La card **no** tiene un control de filtro propio (ni `FilterButton` ni popover): la **leyenda interactiva** (`ChartLegend`, abajo) cumple esa función. En `income-expense` togglea las series Ingresos/Gastos (estado `hiddenSeries`); en `by-category` togglea categorías (`categoryIds`). El stepper de año (pill `‹ año ›`) sí va embebido. El componente recibe el estado (`year`, `categoryIds`, `hiddenSeries`, `categoryChartMode`) y los callbacks de cambio por props, y deja que el anfitrión decida si ese estado es **persistido** (cards de `/reportes`) o **efímero** (dashboard). El componente `CategoryFilterPopover` sigue vivo, pero **no** lo usa la card de reporte: lo usa el filtro por sección de `/mes`.
