...
- **Error:** idéntico a las otras cards — `AlertTriangle` 20px `--warning-ink` + "No se pudo cargar el reporte." + botón ghost "Reintentar", centrado, sin tinte de error.
- **Modo orden / mini de reorden (P1):** **sin tratamiento especial nuevo.** En modo orden la card colapsa al **mismo mini-ítem** de P1 (`[grip] · [ícono tipo] · [título] · ——— · [etiqueta tipo]`). El **ícono de tipo** de `installment-gantt` es **`CalendarRange`** (lucide, 16px `--muted`) — glifo de "rango sobre calendario" que comunica "barras que abarcan un período", distinto del `CalendarDays` de Únicos, del `AreaChart` (income-expense) y del `BarChart3` (by-category); **el mismo glifo** debe usarlo el `AddCardMenu` al ofrecer este tipo (ícono ↔ tipo consistente, igual que los otros). La **etiqueta de tipo** es **"Cuotas"** (12px/600 `--muted`, sentence-case). **Sin mini-preview** del gantt (misma decisión cerrada de P1: identidad por ícono+etiqueta+título). Todo lo demás (caja del mini ~56px, grip, drag in-place, salida en vivo) es idéntico a P1.
