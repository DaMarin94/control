...
> **La leyenda es interactiva: es el filtro.** Desde la Ola 2 (P1), la leyenda **no es decorativa** — cada ítem es un toggle clickeable que activa/desactiva su serie (`income-expense`, dos series Ingresos/Gastos) o su categoría (`by-category`, en ambos modos Barra y Línea). **Reemplaza** al disparador+popover de categorías embebido, que se elimina de la card. El detalle de estados, a11y y layout interactivo vive en *Leyenda interactiva (la leyenda es el filtro)*, abajo.
...
La leyenda **es el filtro** (mismo `ChartLegend` interactivo, *Leyenda interactiva (la leyenda es el filtro)*), pero el universo es **fijo de series** (no categorías abiertas), así que se trata como el caso `income-expense` Total: **pocos ítems, sin `LegendAllChip`** (el atajo "Todas/Ninguna" no aplica — son 3 toggles, no un universo largo; misma economía que las 2 series de income-expense).
...
- **Estados del ítem:** idénticos a la leyenda interactiva del DS (activo/hover/apagado con swatch a outline + label `line-through` + opacidad 0.7 / focus ring `--accent-soft` / pressed `--panel-3`). `role="group"` `aria-label="Filtrar series"` (mismo del caso `income-expense`). Apagar las 3 → canvas vacío → el empty estándar (abajo); la leyenda no se bloquea.
...
- **Leyenda interactiva = filtro de series.** En Total la leyenda son **dos ítems** "Ingresos" (`--income`) / "Gastos" (`--expense`), cada uno toggle de su serie (`hiddenSeries`), tal como ya estaba speceado para el modo Total. **No** hay filtro de categorías en `income-expense` (la descomposición por categoría es ahora exclusiva de `by-category`).
- **Dashboard.** El Dashboard monta `income-expense` y por ende queda **Total-only, sin tabs** — su cabecera pierde igualmente la fila de tabs. Es coherente: el Dashboard nunca persistió el modo (era efímero) y solo mostraba un AreaChart de ingresos vs. gastos; ahora ese es su único render, sin control de vista. El stepper de año (activo) y el filtro de series por leyenda (efímero) no cambian.
...
|---|---|---|
| **`income-expense` (Ingresos vs Gastos) — Total-only** | dos: **Ingresos** (`--income`) / **Gastos** (`--expense`) | esa serie de área. **No hay filtro de categorías** acá. |
...
**La leyenda de `by-category` es la misma en ambos modos.** El toggle Barra↔Línea no cambia la leyenda ni su estado de filtro; cambia solo la geometría del canvas (banda-barra ↔ banda-área). **La lógica de tres estados NO cambia.** El filtro sigue siendo `null`=todas / `[]`=ninguna / lista (subconjunto) para categorías; en `income-expense` Total son dos series independientes que se prenden/apagan. La leyenda solo cambia la **piel** (de decorativa a interactiva) y el **lugar** donde se acciona (deja de haber popover).
...
- **Contenedor:** `role="group"` con `aria-label` por caso — **"Filtrar series"** (`income-expense` Total) / **"Filtrar categorías"** (`by-category`, modos Barra y Línea). Reemplaza el `aria-label="Leyenda del gráfico"` decorativo actual.
