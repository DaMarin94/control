...
- **Línea 2 (`flex flex-wrap items-center justify-between gap-x-4 gap-y-2 mb-[18px]`):**
  ...
  - **Cluster derecho — `CardControls`:** `[ Proyección (chip-toggle, RF-REP-015) ]  [ divisor ]  [ YearStepper ] [ divisor ] [ CardCurrencyTrigger ] [ divisor ] [ X ]`.
...
### Proyección de fijos a futuro en `income-expense` (Ingresos vs Gastos) — RF-REP-015

La card `income-expense` suma un **toggle on/off de proyección de fijos** (default **off**) que, encendido, **extiende las dos series Ingresos/Gastos hacia los meses futuros** trazando un **tramo proyectado** que se **diferencia visualmente** del tramo real. La proyección compone solo los **fijos** en alcance (cuotas y únicos no entran); el **horizonte es ilimitado** (se sigue mostrando al navegar años futuros). El cálculo es de backend; acá vive solo cómo se **ve** el tramo proyectado y cómo se **acciona** el toggle dentro de la cabecera de dos líneas ya cargada por RF-REP-014.

> **Alcance de esta sección.** La card sigue siendo **Total-only, AreaChart de 2 líneas** (*Forma 1 — Ingresos vs. Gastos*): la proyección **no agrega líneas ni ítems de leyenda** — es el **mismo trazo de cada línea, continuado** hacia adelante con piel distinta. El toggle es una **augmentación de vista persistida** (config de card, clave `reports`), **no** un filtro de cómputo: no acota qué movimientos entran, los **extiende**. Por eso vive en `CardControls` (cluster derecho), **fuera de los filtros de cómputo** (Dirección y Tipo en la cabecera; Categoría en el footer) y **subordinado** a ellos (los filtros definen qué fijos hay para proyectar; el toggle decide si se dibuja la continuación).

#### 1. Diferenciación del tramo proyectado — mismo hue, piel "derivada"

El tramo proyectado es la **continuación de la misma serie**, así que conserva su **hue semántico** (Ingresos → `--income` / Gastos → `--expense`): la regla dura 1 (verde = ingreso, rojo = gasto) **manda** — el tramo futuro de gastos **no** puede cambiar de color ni atenuarse a un neutro, sigue siendo rojo gasto. La diferencia real↔proyección se comunica por **patrón de trazo + atenuación de relleno**, reusando **literal** el lenguaje de discontinuidad ya establecido en el chart de tasas (*Inflación vs Ingresos* §1: la serie "derivada/procesada" se separa de la cruda por **dash `6 4`**, no por un segundo hue):

- **Stroke del tramo proyectado:** **2px** (mismo grosor que el real — es la misma serie continuada, no una recta de apoyo), **dasheado `6 4`** (raya larga, idéntico al "derivado de la sólida" del chart de tasas), **opacidad 0.7** (un escalón por debajo del 1.0 del tramo real → "lectura menos cierta, es pronóstico"). El **dash es la señal primaria**; la atenuación a 0.7 refuerza "estimado" sin perder legibilidad. Hue del token de la serie, sin recolorear.
- **Fill del tramo proyectado:** baja de **0.14** (área real) a **~0.05** (área muy tenue) — la región futura se lee "blanda/incierta" mientras el dash carga el valor. Variante aceptable: **gradiente que se desvanece a 0** hacia el borde derecho (más lejos = más incierto); el plano a 0.05 es la opción canónica. El relleno tenue mantiene legibles las **dos** áreas proyectadas cuando se solapan.
- **Curva:** `monotone`, **continua** desde el tramo real (sin quiebre de suavizado en el empalme).
- **Las dos series reciben idéntico tratamiento:** Ingresos = verde `--income` dash `6 4` op .7 fill .05; Gastos = rojo `--expense` dash `6 4` op .7 fill .05. En la región futura se distinguen **por hue**, exactamente como en la región real — el patrón de dash es el mismo para ambas (es "futuro", no "serie distinta").
- **Prohibido:** teñir el tramo proyectado de índigo, atenuarlo a un neutro graphite, o usar un dash distinto por serie. El único delta real↔proyección es **patrón + atenuación**, nunca hue.

#### 2. Punto de empalme real → proyección

El empalme cae en el **mes presente** (el dato medido llega hasta el mes en curso; la proyección arranca el mes siguiente). Se marca con dos piezas neutras de cromo (no income/expense, no índigo):

- **Marca de empalme por serie:** en el **último punto real** de cada serie, un **dot persistente** (radio **3.5px**, relleno = color de la serie, borde `--panel` 2px) — ancla "acá termina lo medido, empieza el pronóstico". Es el mismo vocabulario del `activeDot` (radio 4px, borde `--panel`) pero en reposo y un punto más chico, para no competir con el `activeDot` de hover.
- **Línea divisoria de plot:** una **`ReferenceLine` vertical** en el límite (mes presente), a lo ancho del canvas, **`--line` 1px dasheada `4 4`** (cromo neutro estructural, mismo registro que la baseline del chart de tasas pero discontinua), con un **micro-rótulo** arriba del plot: **"Proyección"** (UI **10.5px / 600 `--faint`**), alineado al lado futuro de la línea. Una sola línea sirve a **ambas** series (las dos empalman en el mismo mes). El dash `4 4` la separa de la gridline `--hair` sólida y la lee como "frontera", no como gridline.
- **Año en curso** (hay split real | proyección): se dibuja la línea divisoria + el rótulo + los dots de empalme. **Año futuro** (todo el canvas es proyección, sin tramo real): **no** hay línea divisoria; en su lugar, un **caption de esquina** arriba-izquierda del canvas — pequeña pieza **`--panel-2` `--r-chip`, texto "Proyección" UI 10.5px/600 `--muted`** — para que se entienda que **todo** el gráfico es pronóstico. **Año pasado** (todo medido): el toggle no agrega tramo (no se proyecta hacia atrás); queda sin efecto visible.

#### 3. Convivencia con la Dirección y los filtros (RF-REP-014)

- **La proyección NO es un ítem de leyenda ni de filtro.** El footer es la leyenda-filtro de **categorías**, no de series: el tramo proyectado **no se prende/apaga por separado** desde ahí. **Sigue la visibilidad de su línea madre** (la línea de ingresos o de gastos según la Dirección; misma economía que las rectas de tendencia del chart de tasas, *Inflación vs Ingresos* §3 — no son ítems propios). Quien controla la proyección es el **chip de cabecera** (§4).
- **Línea fuera de alcance (Dirección):** si la Dirección deja una sola línea (solo gastos / solo ingresos), **el tramo proyectado de la línea ausente no se dibuja** (real + futuro son la misma línea; ya no hay show/hide por leyenda — la Dirección es el único control de cuántas líneas hay, RF-REP-014 §5).
- **Dirección (RF-REP-014 §5):** Dirección define el **alcance**, y por ende el alcance de la proyección. **Solo gastos** → solo la línea de gastos y **su** continuación proyectada; **Solo ingresos** → simétrico; **Ambos** → las dos con sus dos tramos proyectados. **Tipo y Categoría** acotan **qué fijos** se proyectan (solo los fijos en alcance: dirección + categorías tildadas en el footer; los chips de Tipo no excluyen el tramo — la proyección es siempre "solo fijos" por RF-REP-015, pero si el usuario apaga el chip "Fijos", los fijos salen del cómputo **y** de la proyección, que se vacía).
- **Legibilidad con una sola línea:** con una sola línea visible (por Dirección) el tramo proyectado de la otra no se dibuja → no hay solape de dashes; la única línea futura se lee nítida por su hue + dash.

#### 4. Toggle de proyección — chip-toggle en `CardControls`, no en el cluster de filtros

El toggle es **binario, persistido por card, default off**. Reusa **literal** la **caja de chip-toggle** de Tipo (RF-REP-014 §3: `aria-pressed`, on = pieza elevada / off = plana) — **no** se inventa un switch nuevo (el DS no tiene interruptor on/off; su vocabulario binario es el chip-toggle):

- **Forma:** `<button type="button">` `inline-flex items-center gap-[6px]`, **ícono `TrendingUp` 15px** + label **"Proyección"** (UI 12.5px / 600), `px-[10px] py-[5px]`, radio `--r-chip` 7px, transición 0.14s. El ícono lo separa de los chips pelados de Tipo y lo lee como **modo de vista**, no como filtro de inclusión.
- **On (encendida):** fondo `--panel` + borde `1px --line-strong` + `--shadow-sm`, ícono+texto `--ink` (pieza elevada = "activo", mismo "thumb blanco" del DS).
- **Off (default, apagada):** fondo `--panel-2`, borde `1px --line`, **sin sombra**, ícono+texto `--muted` (plano/hundido = "inactivo"). Indistinguible del comportamiento histórico: card recién creada o sin tocar el toggle = off = sin tramo proyectado (back-compat exacto).
- **Hover (sobre off):** ícono+texto → `--ink-2`, borde → `--line-strong`. **(sobre on):** se mantiene elevada, sin cambio extra. **Active/pressed:** `--panel-3`.
- **Focus (teclado):** ring `--accent-soft` 3px (`focus-visible`), radio `--r-chip`. El índigo es cromo de foco, no estado de dato.
- **Ubicación — primer ítem del cluster derecho `CardControls`, antes del `YearStepper`:** `[ Proyección (chip) ]  [ divisor --hair ]  [ YearStepper ]  [ divisor ]  [ CardCurrencyTrigger ]  [ divisor ]  [ X ]`. **Por qué a la derecha y no con los filtros:** (a) **no es un filtro de cómputo** — no acota movimientos, augmenta la vista temporal hacia adelante, así que pertenece al cromo de vista/tiempo de la card (junto al stepper de año que mueve el mismo eje), no a los filtros de cómputo de la cabecera (Dirección · Tipo); (b) un chip **"Proyección" pegado a los chips "Fijos / Cuotas / Únicos"** se **malleería como un cuarto filtro de inclusión de Tipo** — alejarlo de los chips de Tipo elimina esa ambigüedad; (c) es una **preferencia de display persistida por card**, exactamente la familia de `CardCurrencyTrigger`. El `YearStepper` conserva su peso de nav dominante: el chip va **antes** del stepper porque "¿proyecto?" condiciona la lectura del tramo futuro que el stepper revela al avanzar de año.
- **a11y:** `<button>` con `aria-pressed` = on/off, `aria-label="Proyección de fijos a futuro"`. Estado por `aria-pressed` (no por color), coherente con todos los toggles del DS.
- **Dashboard:** la card `income-expense` del Dashboard es **efímera y despojada** (omite moneda y los filtros de RF-REP-014); por el mismo precedente **no monta el toggle de proyección** — es config de la card de `/reportes` que persiste en `reports`.

#### 5. Tooltip del tramo proyectado

Reusa `ChartTooltipContent` (caja `--panel`/`--line`/`--r-ctl`/`--shadow-lg`, *Forma 1 — Tooltip*). Cuando el mes bajo el cursor es **proyectado**, se marca que sus valores son estimados, sin rehacer el tooltip:

- **Encabezado:** mes y año UI 12.5px/600 `--ink` + **sufijo "· Proyección"** (UI 10.5px/600 `--muted`, mismo registro que el rótulo de la línea divisoria) → todo el tooltip se entiende como pronóstico de un vistazo.
- **Filas:** una por serie visible — swatch 8px + nombre `--ink-2` + monto. El monto **sigue siendo dinero**: **mono tabular `tnum`** (regla dura 3), `--income-ink` / `--expense-ink` según serie (color semántico, igual que el tramo real), formato es-AR completo. **Prefijo "≈" (U+2248)** delante del monto, dentro de la misma celda mono, para leer "aproximado/estimado" (`≈ $123.456,00`). El `≈` es un glifo neutro, no rompe el tabular ni tiñe.
- **Mes real:** tooltip idéntico al de *Forma 1* (sin sufijo, sin `≈`). El usuario distingue real↔proyectado por la presencia del sufijo y el `≈`.

#### Restricciones duras reafirmadas

- **Verde = ingreso, rojo = gasto, estrictamente:** el tramo proyectado conserva el hue de su serie (`--income`/`--expense`); **nunca** se neutraliza ni se recolorea. La diferenciación es **patrón (dash `6 4`) + atenuación (op .7 / fill .05)**, no color.
- **Índigo = solo marca/cromo:** aparece únicamente como **focus ring** del chip de proyección. La línea divisoria, los dots de empalme y los rótulos "Proyección" son **neutros** (`--line`/`--faint`/`--muted`/`--panel-2`), no índigo. Ningún monto se tiñe por este control.
- **Mono tabular:** los montos del tooltip proyectado siguen en mono `tnum` con su color semántico; el prefijo `≈` y el eje Y abreviado no cambian. El rótulo "Proyección" y el `· N` no son montos.

> Reutiliza: el **dash `6 4` "versión derivada de la sólida"** y la **atenuación de opacidad** del chart de tasas (*Inflación vs Ingresos*) para el tramo proyectado; la **`ReferenceLine`** + el `activeDot`/dot de borde `--panel` de *Forma 1* para el empalme; la **caja de chip-toggle** (`aria-pressed`, on = elevada / off = plana, focus ring `--accent-soft`) de los chips de Tipo (RF-REP-014 §3) para el toggle; el **`ChartTooltipContent`** de *Forma 1* con su color semántico de monto. Aporta: la regla **"proyección = misma serie continuada, distinta por patrón+atenuación, nunca por hue"**; el **empalme marcado** (línea divisoria neutra + dots + rótulo, con variante caption para años 100% futuros); el **toggle de proyección como augmentación de vista persistida en `CardControls`** (fuera del triet de filtros, para no leerse como 4º filtro de Tipo); y el **marcado de valor estimado** en el tooltip (sufijo "· Proyección" + prefijo `≈`).
