...
- **Dos capacidades nuevas sobre los movimientos fijos**, independientes del resto de v1.1 (no consumen preferencias). RF-MF-005, RF-MF-006 y RN-016 en `docs/requirements.md`; bitácora 2026-06-16; spec visual en `docs/design.md`.
- **P1 — anular un fijo en un mes puntual (RF-MF-005):** desde el menú de acciones del ítem fijo en `/mes`, acción **toggle "Anular / Des-anular este mes"** (solo en fijos; únicos y cuotas no la tienen). El mes anulado **se sigue mostrando** con diferenciación visual, **no suma** a los totales del mes ni a la proyección anual. Se modela con la entidad **`RecurringSkip`** `(recurringId, month)` con unicidad y `onDelete: Cascade` — **distinta de `deletedFrom`**.
