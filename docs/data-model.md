...
MovementItem = {
  ...
  hasCalculated: boolean                     // true si el ítem es un fijo ORIGEN con ≥1 calculado en el mes (padre); false en el resto (1.1.7)
}

CalculatedInfo = {                           // Fase 1.1.7 — solo en ítems que son calculados
  sourceChainId: string,                     // chainId del fijo de origen (vínculo a la cadena, no a una fila)
  sourceId: string,                          // id de la fila activa del origen en el mes (para prefilear el form)
  ...
  sourceAmountCents: number                  // monto del origen en el mes (centavos, > 0); base de la fórmula para el preview
}
```
...
- **Movimientos calculados — son `origin: "fijo"` (Fase 1.1.7).** Un movimiento calculado viaja en la lista `fijos` con `origin: "fijo"` (es un fijo a todos los efectos): su `amountCents` ya viene **derivado al vuelo y con signo** para el mes (`signo × round(fórmula(montoOrigenEseMes))`, RN-017/018) y puede ser **≤ 0**; su `type` viene **derivado de ese signo** (negativo → `EXPENSE`, positivo → `INCOME`, cero → `EXPENSE`; RN-018), no es un dato elegible. La **relación padre/hijo** (RF-MCALC-007) se expone con dos campos del `MovementItem`: **`calculated`** (objeto `CalculatedInfo` si el ítem **es** un calculado / hijo, `null` si no) y **`hasCalculated`** (`true` si el ítem es un fijo **origen** con ≥1 calculado derivado en ese mes / padre). Un calculado nunca es padre (`hasCalculated: false` siempre — sin encadenamiento). El **orden de la lista `fijos`** usa la **magnitud** = `\|amountCents\|` DESC (ver "Orden de las listas"): un calculado negativo ordena por su tamaño, no al final.
...
## Contrato de movimientos calculados (Fase 1.1.7)

Endpoints propios del calculado, en el `RecurringModule` (el calculado **es** un fijo). **JWT requerido**; scope por `userId` del token. Mecánica de implementación (cadena, derivación, cascada) en `docs/backend.md`, §Movimientos calculados; reglas funcionales en `requirements.md`, RF-MCALC-001..007 y RN-017/018/019.
...
- **`:id` del POST = id de la fila activa del fijo de ORIGEN** (el ítem desde el que se dispara la acción en `/mes`). El calculado se crea como una cadena `Recurring` nueva vinculada a la cadena del origen.
- **El body NO acepta `type`** (en ninguno de los dos): el tipo se **deriva del signo** del monto al vuelo (RF-MCALC-003); si el front lo envía, la whitelist de class-validator lo descarta.
- **`formulaOperator`** ∈ `{ ADD, SUB, MUL, DIV, PCT }`. **`formulaSign`** ∈ `{ 1, -1 }`. **`formulaOperand`** es un **entero escalado** (ver "Escalado del operando" abajo).
- **`POST` — errores:** `400` si el origen es **a su vez un calculado** (sin encadenamiento), si `formulaOperand = 0` con `DIV`/`PCT` (división por cero, RN-017), o si la categoría es inválida (inexistente/ajena/eliminada). `404` si el origen no existe o no es del usuario. La categoría se valida con scope `BOTH` (`skipScopeCheck`) porque el tipo del calculado es derivado y no fijo.
- **`PATCH` — split y errores:** `currentMonth` (`YYYY-MM`) es **requerido**; usa la **misma mecánica de split del pasado** que `PATCH /recurring/:id` (ver `docs/backend.md`, §Inmutabilidad del pasado). Editables: `categoryId`, `description`, `formulaOperator`, `formulaOperand`, `formulaSign`. **No** editable el vínculo al origen. `400` si el `:id` **no es un calculado**, o `formulaOperand = 0` con `DIV`/`PCT`. `404` si no existe o no es del usuario.
- **`PATCH /recurring/:id` (fijo normal) rechaza con `400` si el `:id` es un calculado** (y viceversa, `PATCH /recurring/:id/calculated` exige que lo sea): cada tipo se edita por su endpoint.
- **`DELETE /recurring/:id` opera sobre la cadena completa y cascadea a calculados.** Borrar un fijo aplica el `boundary` (ver `docs/backend.md`, §Eliminación) a **todas** las filas de su cadena (`chainId`); si el `:id` es un fijo **origen** (no calculado), la misma operación **cascadea** a las cadenas de sus calculados (`sourceChainId = chainId del origen`) con el mismo `boundary`. Borrar un **calculado** no toca el origen ni a otros calculados (RF-MCALC-006).
...
- **`sourceChainId`** — `null` en fijos normales; el **`chainId` del origen** en calculados (vínculo a la cadena, no a una fila).
