...
| **Simulación de categoría (`Simulation`)** | Configuración que proyecta, a lo largo de un **tramo propio** (`startMonth`/`endMonth`), el total de **movimientos únicos** de una categoría (RF-SIM-001..005). Persiste `userId` + `categoryId` + el **tramo**: ni monto ni dirección se persisten — se derivan al vuelo (RN-028). `endMonth` es editable —se extiende hacia adelante, RF-SIM-005—; `startMonth` es inmutable. **Borrado físico**: no participa del historial ni es deshacible (RF-SIM-004). Ver §Simulación de categoría. |
...
ReportCardConfig = {
  ...
  fixedRangeMonths?: 3 | 6 | 9 | 12 | 24 | 36 | 48 | 60  // solo fixed-evolution: largo del rango en meses (RF-REP-013); ausente = 36
}
...
- **`includeSimulated`** (RF-REP-017) — toggle de **movimientos simulados** de la card, solo en `type: "income-expense"` y `"by-category"`. **Ausente = apagado** (back-compat: una card sin el campo grafica solo lo real; el blob **no se migra**). Es lo que el front traduce al query param `includeSimulated` de `GET /movements/reports` (ver contrato más abajo). Es **independiente** de `projectFixed` y del resto de los filtros. Las demás cards lo ignoran; la card del dashboard no lo usa.
...
- **`fixedSelectedIds`** (RF-REP-013) — **solo `fixed-evolution`**: selección de gastos fijos por **`chainId`**. **`null` = todos** los fijos del universo de la leyenda del rango (default al crear); una **lista** = subconjunto explícito (puede ser `[]` = ninguno). **Reemplaza al filtro de categorías** (esta card no lo tiene) y se aplica **client-side** sobre las `lines` del response — no viaja al endpoint. **Sobrevive a los cambios de rango**: un `chainId` que sale del universo se conserva en la lista (no se dibuja, y vuelve a dibujarse si el rango lo reincorpora); la navegación **nunca reescribe** el campo. Persistido por card.
- **`fixedRangeMonths`** (RF-REP-013) — **solo `fixed-evolution`**: **largo del rango visible en meses**, uno de `3 | 6 | 9 | 12 | 24 | 36 | 48 | 60` (las ocho opciones del select: 3/6/9 meses y 1/2/3/4/5 años). **Ausente = `36`** (3 años), también para las cards ya persistidas (el blob **no se migra**). El rango pedido es el que **termina en el mes en curso** y cubre `fixedRangeMonths` meses corridos hacia atrás, antes del recorte del backend contra el primer mes con gasto fijo. Las demás cards ignoran el campo. Persistido por card.
...
- **El back NO valida ni conoce esta clave** (igual que `monthSections`): `PUT /preferences` guarda el blob tal cual. La normalización y los defaults son del frontend consumidor. Es lo que permite que cada tipo de card sume **campos propios** (`anchorUsdCents`, `fixedMode`, `fixedAdjusted`, `fixedSelectedIds`, `fixedRangeMonths`) sin tocar el servidor ni migrar blobs existentes.
...
- **`today`** (opcional) — fecha local del usuario, `YYYY-MM-DD`; formato inválido → `400`. Fija el **mes en curso** con el que se resuelven el tramo (por simulación) y la ventana histórica de las simulaciones (RN-028) y, por lo tanto, qué meses traen movimientos simulados. **El frontend lo manda siempre** con la fecha local del navegador (ver `docs/frontend.md`, §Datos (`use-movements`)); **ausente** el backend cae a la **fecha UTC del sistema** (mismo criterio que el resto de los endpoints con este param), que en zonas UTC−N adelanta el mes en curso y hace aparecer movimientos simulados sobre el mes en curso real. Irrelevante para el resto de la respuesta.
...
MovementItem = {
  ...
  chainId: string | null,                    // identidad de cadena PROPIA del ítem; solo origin==="fijo" (incluye calculado de fijo); null en único y cuota
...
- **`chainId` — identidad de cadena propia del ítem.** No-null **solo** con `origin === "fijo"`: fijos normales y calculados de fijo, cada uno con **su propia** cadena — **nunca** el `sourceChainId` del origen (ese viaja en `calculated`). Es **`null`** en únicos y cuotas, **incluidos sus calculados** (que se persisten como `Recurring` pero se editan siempre in-place, sin split) y los ítems **simulados**: esas formas no tienen concepto de cadena y su clave estable es su propio `id`. Es la **clave de agrupación estable de un fijo a través de los splits de edición** —el `id` de fila cambia en cada split, el `chainId` no—, la misma razón por la que el `targetId` de un `HistoryEntry` de fijo es el `chainId` (ver §Historial de cambios). Consumo en `docs/frontend.md`.
...
- **`simulated` — el movimiento simulado viaja mezclado en `movements.unicos`** (RF-SIM-003). No es una lista aparte: viene **mezclado y reordenado** junto a los únicos reales por el mismo criterio de magnitud DESC (desempate por `occurredAt` DESC, que un simulado no tiene y por eso cede en un empate exacto), participa del filtro `categories` y **suma a `totals`** como cualquier único. Solo puede darse con `origin: "unico"` y `simulated: true`; **todo ítem real trae `simulated: false`**.
  ...
  - **El corte de tramo se resuelve server-side, por simulación.** Un mes pasado o fuera del tramo de esa simulación (RN-028) simplemente no trae su ítem simulado: el consumidor no filtra por mes ni recalcula el tramo para decidir qué mostrar. El **mes en curso sí** puede traer ítems —es el arranque efectivo mínimo de cualquier tramo— y cada ítem es el **remanente** ya neteado contra los únicos reales de ese mes (RN-028), no un monto a sumar aparte.
...
| `PATCH /installments/:id/calculated` | `{ currentMonth, categoryId?, description?, formulaOperator?, formulaOperand?, formulaSign? }` | `200` · `data: Recurring & { historyEntryId }` | `400` · `404` |
...
ReportsSimulatedBlock = {
  months: ReportSimulatedMonth[],       // SIEMPRE 12 entradas, mismo índice/orden que `months`
  categories: ReportSimulatedCategory[] // desglose de GASTO simulado; solo categorías con aporte
}

ReportSimulatedMonth = {
  month: string,                      // "YYYY-MM"
  incomeCents: number,                // aporte simulado de ingreso del mes; 0 = sin aporte
  expenseCents: number                // aporte simulado de gasto del mes; 0 = sin aporte
}

ReportSimulatedCategory = {
  categoryId: string,
  name: string,
  color: string,                      // "#rrggbb"
  monthlyExpenseCents: (number | null)[]  // 12 valores ene→dic; null = SIN aporte ese mes (≠ 0)
}
...
- **`simulated.months` — siempre 12 entradas**, mismo índice y orden que `months`. `incomeCents`/`expenseCents` en **0** donde no hay aporte (mes pasado, fuera del tramo de todas las simulaciones, simulación pausada, remanente cancelado o dirección opuesta). El **mes en curso sí puede traer aporte**: es el arranque mínimo de cualquier tramo, y su aporte es el **remanente** del mes (RN-028), con lo que ese mes queda **mixto** (real a la fecha + remanente).
...
- **`rangeMonths`** (opcional) — largo del rango **pedido**, en meses: una de las **8 opciones** `3 | 6 | 9 | 12 | 24 | 36 | 48 | 60`. **Ausente → `36`**; cualquier otro valor → **`400`**. No hay año ni mes final que pedir: el **borde derecho es siempre el mes en curso**, que el backend resuelve con `today`. El pedido es **previo al recorte** del borde izquierdo.
- **`currency`** (opcional) — override de la moneda de display (RF-REP-007), una de las 4 monedas, **case-sensitive**. **Ausente → la default global del usuario**; **presente y válido → esa moneda**; **vacío o fuera del set → `400`**.
...
FijosHistoricoResponse {
  ...
  endMonth: string;                   // "YYYY-MM" — SIEMPRE el mes en curso
  lines: FixedEvolutionLine[];        // ≥2 apariciones graficables; orden: gasto del rango DESC, desempate chainId ASC
  excluded: FixedEvolutionExcludedLine[]; // cadenas con 0 o 1 apariciones en el rango
}

FixedEvolutionLine {
  chainId: string;
  ordinal: number;                    // rank estable (ver abajo)
  ...
  months: FixedEvolutionMonthPoint[]; // largo VARIABLE = rangeMonths efectivo (3..60)
}
...
FixedEvolutionExcludedLine {
  ...
  categoryName: string;
  categoryColor: string;              // "#rrggbb"
  startMonth: string;                 // "YYYY-MM" — único dato adicional del excluido
}
```

- **El rango viaja resuelto: `startMonth`, `endMonth` y `rangeMonths` describen lo que la respuesta REALMENTE cubre.** `rangeMonths` es el largo **efectivo**, después de recortar el borde izquierdo contra el primer mes con gasto fijo del usuario, y puede ser menor al pedido; el borde derecho nunca se corre para compensar. El front rotula el eje y los estados con el rango efectivo, no con lo que pidió. No hay topes de navegación que transportar: hacia adelante no hay nada que topar (`endMonth` es el mes en curso) y el límite hacia atrás ya está expresado en `startMonth`, con granularidad de mes.
- **`months` es de largo variable y cada punto se identifica por su `month`.** El índice del array no dice de qué mes es un punto. **Ningún punto es posterior a `endMonth`.**
...
> Destino canónico del modelo y del contrato de API de la simulación. Reglas funcionales en `requirements.md`, módulo 3.15 (RF-SIM-001..005) y RN-028/029; el movimiento simulado que la simulación aporta a `/mes` está en §Contrato de movimientos del mes (`simulated`); implementación en `docs/backend.md`, §Simulación de categoría.
...
Simulation = {
  ...
  startMonth: string,      // "YYYY-MM" — mes desde el que se creó, ya clampeado contra el mes en curso; inmutable
  endMonth:   string,      // "YYYY-MM" — fin del tramo; persistido, no derivado (RN-028/029); se corre hacia adelante al extender (RF-SIM-005)
  createdAt: DateTime
}
```

- **Persiste la configuración y el tramo** (`userId` + `categoryId` + `startMonth` + `endMonth`, RN-029). No guarda monto, dirección ni ventana: se derivan al vuelo en cada lectura (RN-028). **No genera filas de movimiento** — el movimiento simulado se sintetiza en la respuesta de `GET /movements`.
- **`startMonth`** = `max(mes desde el que se crea, mes en curso)`, fijado al crear y **nunca cambia**. **`endMonth`** = diciembre del año de `startMonth`, extendido a `startMonth + 6` si ese tramo queda por debajo de 6 meses (RN-028); se **persiste** en vez de derivarse porque es un extremo **movible**: `PATCH /simulations/:id/extend` (RF-SIM-005) lo corre hacia adelante sin volver a aplicar esa fórmula. El **arranque efectivo** de una lectura (`max(startMonth, mes en curso de esa lectura)`) no se persiste — se calcula on-the-fly y viaja como `effectiveStartMonth` en el DTO.
...
**JWT requerido** en los cinco; scope por `userId` del token (`401` global si falta o es inválido).

**`today`** (`YYYY-MM-DD`, opcional) es query param de los **cuatro** endpoints que calculan —`POST /simulations`, `GET /simulations`, `GET /simulations/candidates` y `PATCH /simulations/:id/extend`— con la misma semántica que en `GET /movements`: fija el mes en curso con el que se resuelven la **ventana histórica de 12 meses**, el **tramo** de cada simulación y, en `PATCH .../extend`, los derivados (`monthsWithData`/`paused`/`effectiveStartMonth`) del `SimulationDto` de respuesta (RN-028). Formato inválido → **`400`**; **ausente = fecha UTC del backend**. `DELETE /simulations/:id` no lo acepta (no calcula nada).

**`startMonth`** (`YYYY-MM`, opcional) tiene dos usos distintos según el endpoint:
- En `POST /simulations`, es el mes desde el que se crea el lote (el mes visualizado en `/mes`); ausente = mes en curso. Se aplica a **todo el lote** y el backend lo **clampea** contra el mes en curso (RN-028) — no hace falta que el caller lo clampee. Formato inválido (no matchea `/^\d{4}-\d{2}$/`) → `400`.
- En `GET /simulations/candidates`, es el mismo mes hipotético, usado solo para **previsualizar** el tramo (`startMonth`/`endMonth` de la raíz de la respuesta) que resultaría si se creara un lote desde ahí; no crea nada.

- **GOTCHA — `today` es opcional en el contrato pero obligatorio en la práctica: todo consumidor tiene que mandarlo, en los tres.** El fallback UTC no es equivalente a la fecha local: en UTC−N, cerca de fin de mes, el backend ya está en el mes siguiente mientras el usuario todavía no. Una superficie que lo omita resuelve ventana y tramo con un mes distinto al de las que sí lo mandan, y las superficies **se contradicen entre sí** — el mismo `monthsWithData` / `effectiveStartMonth` difiere según qué endpoint lo produjo, y una categoría elegible en el selector puede ser rechazada por el `POST`. Omitirlo no rompe nada de forma visible: la respuesta es válida, solo está corrida un mes.

| Endpoint | Body | Éxito | Errores |
|---|---|---|---|
| `POST /simulations` | `{ categoryIds: string[], startMonth?: "YYYY-MM" }` | `201` · `data: CreateSimulationsResponse` | `400` |
| `GET /simulations` | — | `200` · `data: SimulationsListResponse` | — |
| `GET /simulations/candidates` | — | `200` · `data: SimulationCandidatesResponse` | — |
| `PATCH /simulations/:id/extend` | `{ months: 1 \| 3 \| 6 \| 12 }` | `200` · `data: SimulationDto` | `400`, `404` |
| `DELETE /simulations/:id` | — | `204` sin body | `404` |

- **`POST /simulations` es un ALTA MÚLTIPLE con fallo parcial tolerado (RF-SIM-001).** El body es `{ categoryIds: string[], startMonth?: "YYYY-MM" }`: `categoryIds` **no vacío** y **sin ids duplicados**, y `startMonth` (si viene) debe matchear `/^\d{4}-\d{2}$/` — cualquiera de las tres cosas es `400` de body inválido, los **únicos** `400` del endpoint. Fuera de eso responde **`201` siempre**, incluso si fallaron **todas** las categorías: un fallo por categoría no es un error duro de la operación, viaja en `failed`.
- **Los `message` de `failed` son los textos legibles por categoría**, uno por causa de rechazo: categoría inexistente, ajena o eliminada (mismo criterio **no revelador** que el resto de los movimientos: nunca `404`); categoría con **menos de 3 meses con únicos** en la ventana (RF-SIM-002); categoría que **ya tiene una simulación** (existencia según el índice único `(userId, categoryId)`, sin mirar `paused`). El selector ya **oculta** esas categorías (no las ofrece, ni siquiera deshabilitadas), así que solo se alcanzan en una carrera con candidatas stale (universo congelado al abrir el selector, RF-SIM-001).
- **`PATCH /simulations/:id/extend` corre el `endMonth` hacia adelante** (RF-SIM-005): `endMonth` nuevo = `endMonth` vigente + `months`. **No** se re-deriva con `computeHorizonEndMonth` (exclusiva de la creación), **no** se clampea contra el mes en curso y **no** tiene tope. `months` ausente o fuera de `{1, 3, 6, 12}` → `400`; simulación inexistente o ajena → `404`. Responde el mismo `SimulationDto` de siempre, con `monthsWithData`, `paused` y `effectiveStartMonth` recalculados.
...
SimulationDto = {
  ...
  startMonth: string,             // "YYYY-MM" — mes desde el que se creó, ya clampeado, inmutable
  effectiveStartMonth: string,    // "YYYY-MM" — max(startMonth, mes en curso de ESTA lectura)
  endMonth: string,                // "YYYY-MM" — fin del tramo, persistido
...
SimulationCandidatesResponse = {
  startMonth: string,             // "YYYY-MM" — arranque del tramo hipotético de un lote creado ahora
  endMonth: string,               // "YYYY-MM" — fin de ese tramo hipotético
  categories: Array<{
    categoryId: string,
    name: string,
    color: string,
...
- **El tramo es propio de cada simulación, no de la respuesta.** `startMonth` se persiste tal cual quedó fijado al crear y nunca cambia; `endMonth` también se persiste (RN-028/029), porque es el extremo que se corre hacia adelante al extender (RF-SIM-005). `effectiveStartMonth` sí es un derivado vigente: `max(startMonth, mes en curso de esta lectura)`, recalculado contra el `today` de cada llamada. Dos simulaciones pueden tener `startMonth`/`endMonth` distintos entre sí; no hay un tramo único a nivel respuesta.
- **`GET /simulations/candidates` expone `startMonth`/`endMonth` en la raíz como previsualización de un lote hipotético**, no como propiedad de una categoría puntual — todavía no existe ninguna simulación de ese lote. Refleja el mismo cálculo que aplicaría `POST /simulations` si se confirmara con ese `startMonth` (o el mes en curso si se omite).
...
### `historyEntryId` — id de la entrada en las respuestas de edición y eliminación

Todo endpoint que **edita** o **elimina** un movimiento devuelve el **id de la entrada de historial que esa operación generó**. Es lo que habilita deshacer sin consultar `GET /history`: el id se le pasa directo a `POST /history/:id/undo` (RF-HIST-007).

| Endpoint | Éxito |
|---|---|
| `PATCH /transactions/:id` | `200` · `data: Transaction & { historyEntryId }` |
| `PATCH /recurring/:id` | `200` · `data: Recurring & { historyEntryId }` |
| `PATCH /recurring/:id/calculated` | `200` · `data: Recurring & { historyEntryId }` |
| `PATCH /transactions/:id/calculated` | `200` · `data: Recurring & { historyEntryId }` |
| `PATCH /installments/:id/calculated` | `200` · `data: Recurring & { historyEntryId }` |
| `PATCH /installments/:id` | `200` · `data: InstallmentGroup & { historyEntryId }` |
| `DELETE /transactions/:id` | `200` · `data: { historyEntryId }` |
| `DELETE /recurring/:id` | `200` · `data: { historyEntryId }` |
| `DELETE /installments/:id` | `200` · `data: { historyEntryId }` |

- **En los `PATCH` el merge es plano:** `historyEntryId` es una propiedad más al lado de los campos de la entidad, no un objeto anidado ni un sobre extra.
- **Los `DELETE` de movimientos responden `200` con cuerpo, no `204`:** un `204 No Content` **no puede llevar body por spec HTTP**, y la respuesta tiene que traer el id de la entrada. Son la excepción a la convención de DELETE del backend (ver `docs/backend.md`, §DELETE → `204 No Content`).
- **Los `POST` (creación) no lo devuelven:** crear no genera entrada de historial (RF-HIST-001).
