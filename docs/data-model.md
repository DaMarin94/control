...
ReportCardConfig = {
  ...
  projectFixed?: boolean,                        // toggle de proyección de fijos a futuro (income-expense, RF-REP-015); ausente = off
...
- **`projectFixed`** (RF-REP-015) — toggle de **proyección de fijos a futuro** de la card `income-expense`: `true` extiende las series a los meses futuros proyectando solo los fijos (ver §Contrato de serie de reportes para el efecto en la respuesta). **Ausente o `false` = off** (back-compat: una card sin el campo no proyecta). Solo aplica a `type: "income-expense"` de `/reportes` (el dashboard no lo monta). Persistido por card.
...
MovementItem = {
  ...
  skipped: boolean,                          // fijos: true si anulado para el mes; calculado de fijo: skip propio OR skip del fijo de origen; únicos, cuotas y calculados de único/cuota: false
...
- **`frequency` / `skipped`.** Para un **fijo**, `frequency` es su periodicidad (`MONTHLY`…`ANNUAL`, RF-MF-006) y `skipped` indica si esa aparición está **anulada para el mes consultado** (RF-MF-005). Para un **calculado de fijo**, `frequency` es **`null`** y `skipped = skip propio del calculado **OR** skip del fijo de origen` (RF-MF-005 / RF-MCALC-005): un calculado de fijo se puede anular por su cuenta y además hereda la anulación de su origen. Para **únicos**, **cuotas** y **calculados de único o de cuota**, `frequency` es **`null`** y `skipped` es **`false`** (no aplican). Un ítem con `skipped: true` **viene igual en la lista** (no se omite) pero **no está incluido en `totals`** — el front lo muestra con su diferenciación visual de anulado. El front usa `frequency` para rotular la cadencia y `skipped` para el estado y el toggle de la acción anular/des-anular.
...
- **`projectFixed`** (opcional) — proyección de **fijos a futuro** (RF-REP-015): `"true"` activa la proyección; **ausente o cualquier otro valor = off** (back-compat: respuesta idéntica a la de sin el param). Con `projectFixed=true` los meses futuros (posteriores a `today`) extienden las series proyectando solo los fijos (regla de cálculo en `docs/backend.md`). Lo usa solo la card `income-expense` de `/reportes`.
...
- **Mapeo persistencia→query** (el front deriva los params del blob de la card): `movementTypes` ausente **o** los tres tipos → se **omite** `types`; `[]` → `types=` vacío; subconjunto → CSV. `direction` ausente **o** `both` → se **omite** `direction`; `expense`/`income` → el valor literal. `projectFixed` ausente/`false` → se **omite** el param (no se manda `projectFixed=false`); `true` → `projectFixed=true` **más** `today`. `today` se manda **solo** con `projectFixed=true` (omitirlo cuando la proyección está off evita refetches espurios al cambiar la fecha). (`categoryIds` → `categories` como ya se describe arriba.)
...
ReportMonth = {
  ...
  projected: boolean                  // true SOLO en meses futuros (> today) con projectFixed=true; con la proyección off, false en todos
}
...
- **`projected` — corte real vs. proyectado a futuro (RF-REP-015).** Con `projectFixed=true`, los meses posteriores a `today` vienen con `projected: true` y sus totales suman, además del dato real, la **proyección de los fijos a futuro** (solo fijos; cuotas y únicos no se extienden — regla de cálculo en `docs/backend.md`). Los meses ≤ `today` y, con la proyección off, **todos** los meses vienen `projected: false`. El front usa este flag para separar visualmente el tramo proyectado del real.
