...
| `Simulation` | Simulación de categoría: `userId` + `categoryId` + tramo propio (`startMonth`, `endMonth`). **Borrado físico** e índice único `(userId, categoryId)`. Modelo y contrato en `docs/data-model.md` §Simulación de categoría. |
...
|--------|-----------|-------------|
| `movements` | `GET /movements`, `GET /movements/reports`, `GET /movements/reports/annual-unicos`, `GET /movements/reports/annual-cuotas`, `GET /movements/reports/annual-inflation-income`, `GET /movements/reports/fijos-historico` | Lista unificada del mes + serie de reportes agregada + grilla anual de Únicos + gantt anual de Cuotas + series anuales de Inflación vs Ingresos + serie histórica por gasto fijo (transacciones + recurrentes + cuotas) |
...
### `GET /movements/reports/fijos-historico?rangeMonths=&currency=&today=`
Devuelve **una serie de los últimos N meses por gasto fijo** (monto + variación nominal + variación ajustada por inflación) para la card `fixed-evolution` (RF-REP-013). Solo agrega movimientos de tipo **Fijo** y dirección **gasto (`EXPENSE`)**. **No acepta `categories`** (la card no filtra por categoría). Contrato (params y shape) en `docs/data-model.md`, §Contrato del detalle histórico de Fijos; reglas de cálculo en la sección **Movimientos del mes (MovementsModule)** → Detalle histórico de Fijos.
...
Una serie de **los últimos `rangeMonths` meses hasta el mes en curso**, **por gasto fijo**, para la card `fixed-evolution` (RF-REP-013), scopeado por `userId` del JWT. Contrato (params, shape `FijosHistoricoResponse` / `FixedEvolutionLine` / `FixedEvolutionMonthPoint` / `FixedEvolutionExcludedLine`) en `docs/data-model.md`, §Contrato del detalle histórico de Fijos. Reglas de negocio:
...
- **El rango se ancla al presente y solo se estira hacia atrás.** El borde derecho (`endMonth`) es el **mes en curso** resuelto con `today`; el izquierdo, ese mes menos `rangeMonths - 1`. No hay navegación: no se pide ni se devuelve un período suelto del pasado.
- **Ningún punto posterior al mes en curso, aunque el monto sea conocido o proyectable.** Un fijo vigente tiene monto determinado para los meses futuros (RN-016) y el sistema sabe proyectarlo (RN-006), pero este reporte es **histórico**: la serie se corta en `endMonth`. Es una decisión, no un límite técnico — no "arreglar" extendiendo la serie a futuro.
- **Recorte del borde izquierdo contra el primer mes con gasto fijo del usuario.** Si se pide más rango del que hay historia, la serie arranca en ese primer mes y el rango sale **más corto** que el pedido; el borde derecho **no se corre** para compensar. La respuesta devuelve el rango **efectivo** (`startMonth`, `endMonth`, `rangeMonths`), y es contra ese largo que se dimensionan todos los `months`.
...
- **Resolución del origen de un calculado — sin queries adicionales.** `originDescription` / `originChainId` se resuelven contra el **universo completo de `Recurring` del usuario** ya cargado, no contra `lines`. Es obligatorio que sea así: el `sourceChainId` puede apuntar a una cadena **ausente de `lines`** —un fijo de **ingreso**, o un fijo sin apariciones en el rango pedido—, y buscarlo solo entre las líneas emitidas lo dejaría irresuelto.
  - **Gotcha estructural — un origen totalmente irresoluble deja al calculado sin apariciones.** Si el `sourceChainId` no se resuelve en ningún lado (su cadena de origen fue eliminada por completo), el fallback vigente deja al calculado **sin monto en ningún mes**: con 0 apariciones no entra en `lines` y sale por **`excluded`**. Consecuencia observable: **una línea de `lines` nunca tiene `originDescription: null`**.
...
- **Las dos variaciones se calculan entre PAGOS CONSECUTIVOS de la cadena, no entre meses consecutivos.** El término de comparación de un punto es **la aparición previa de esa misma línea** (monto no-null), cualquiera sea la distancia en meses: los meses sin aparición por frecuencia y los **anulados no cortan la cadena** —se compara contra el último pago real previo—. Para un fijo **mensual** el pago anterior *es* el mes anterior, así que el resultado coincide con el mes a mes. **Racional:** comparar contra el mes anterior del calendario dejaría a toda cadena de `frequency > 1` **sin un solo punto** en los modos de variación, porque su mes anterior nunca tiene monto.
- **El pago anterior puede caer fuera del rango pedido y se usa igual**, para que el punto de `startMonth` también tenga variación. Ese pago se convierte con **la cotización de su propio mes** y **no viaja** en la serie.
- **Gotcha — ventana interna de lookback de 24 meses (`VARIATION_LOOKBACK_MONTHS`).** Para resolver pagos anteriores que caen antes de `startMonth`, el cálculo trabaja sobre un tramo extendido **24 meses hacia atrás del borde izquierdo efectivo**, y carga cotizaciones e IPC de ese tramo extendido, no solo del rango pedido. **24 = el doble de la frecuencia máxima soportada** (12, RF-MF-006): alcanza incluso si el pago inmediatamente anterior de un fijo anual estaba anulado. La ventana es **interna al cálculo y no se expone en la respuesta**: el contrato sigue describiendo solo el rango efectivo.
- **Cotizaciones e IPC se cargan por LISTA DE MESES** (los del rango efectivo más los de la ventana de lookback), no por año calendario: el rango arranca en cualquier mes y puede cruzar varios años.
- **El mes en curso participa de la variación como cualquier otro** (asimetría deliberada con `annual-inflation-income`, que anula la variación del mes en curso). Un fijo es **determinístico** (RN-016): su monto del mes ya está definido, no es una medición incompleta como el total de ingreso. `nominalPct` / `adjustedPct` solo son `null` por falta de pago anterior (o de IPC del tramo, en la ajustada).
- **`adjustedPct` — variación nominal menos el IPC ACUMULADO COMPUESTO del tramo entre los dos pagos.** El tramo son los meses que van del pago anterior al pago del punto; se compone la `InflationRate.monthlyVariation` (puntos %, unidad canónica; ver Reporte anual de Únicos) de cada uno de esos meses. **Es todo o nada:** si falta la fila de IPC de **cualquier** mes del tramo, `adjustedPct` sale `null` — descontar una inflación parcial daría un resultado engañosamente favorable. La respuesta **no identifica** cuál mes faltó.
- **`lines` = cadenas con ≥2 apariciones GRAFICABLES en el rango efectivo.** Una **aparición graficable** es un mes con monto (`amountCents` no-null, incluido el `0` real). **No cuentan** los meses sin aparición por frecuencia, los anulados, los previos al alta, los posteriores a la baja, ni el mes en que un calculado resulta `INCOME` — es decir, ningún hueco con `reason`. Una línea de un solo punto no dibuja evolución, que es lo que la card muestra. El criterio se evalúa sobre las apariciones y **no depende del modo de visualización** (que es del front). `lines` se ordena por **gasto total del rango** desc, desempate `chainId` asc.
- **`excluded` = TODAS las cadenas de gasto fijo del usuario con 0 o 1 apariciones en el rango**, con su `startMonth`. Incluye las de **vigencia terminada hace años** y las de **alta futura** (`startMonth` posterior al mes en curso), que por definición no tienen apariciones en el rango. Emitirlas es lo que mantiene alcanzable a un fijo que existe pero no dibuja: un fijo de alta futura —y un calculado cuyo fijo de origen ya no se puede resolver— **aparecen en `excluded`**, no desaparecen del reporte.
- **`ordinal` se computa sobre el universo COMPLETO del usuario**, no sobre el rango pedido (que solo determina qué cadenas entran en `lines` y cuáles en `excluded`). Es el rank por la fila más antigua de cada cadena: **tiene que ser estable entre rangos** porque es la identidad con la que el front asigna color; si dependiera del tramo mostrado, un fijo cambiaría de color al cambiar el rango.
...
- **La batch hace DOS cargas de datos por llamada, no una por mes ni una por simulación.** Una es la **ventana histórica `[A-12..A-1]`** (serie de ajuste + `monthsWithData`/`paused`); la otra, los **únicos reales que el remanente descuenta** (RN-028), sobre el rango que resulta de la **unión de los tramos vigentes de todas las simulaciones del usuario** (cada tramo va de su `effectiveStartMonth` a su `endMonth`), intersecada con los meses pedidos y **reducida a un rango contiguo** (min/max) — `loadCategoryMonthlyData` solo necesita los extremos para acotar su query. Cada carga es **una sola query**: el costo sigue siendo **O(1) por llamada**, no O(meses) ni O(simulaciones).
- **Si ninguna simulación cubre ninguno de los meses pedidos, la batch corta antes de tocar la DB** (ni la ventana histórica ni el rango real se cargan): todas vencidas o con tramos que no alcanzan los meses pedidos es un `Map` de `[]` sin queries.
- **Las dos cargas no se fusionan en una.** El helper que las resuelve acumula en el mismo `monthsWithData` **todo** lo que devuelve la query; mezclar los dos rangos inflaría el conteo de "meses con dato" de la ventana histórica con meses del rango real y falsearía el mínimo de 3 meses.
...
- **`getSimulatedItemsForMonth` corta temprano y devuelve `[]`** si el mes pedido es **anterior** al mes en curso, si cae fuera del **tramo de esa simulación** (`[effectiveStartMonth..endMonth]`), si el usuario no tiene simulaciones o si la simulación está **pausada** (`monthsWithData < 3`). El **mes en curso no corta por sí solo**: es el piso de `effectiveStartMonth` de cualquier tramo. El consumidor no evalúa ninguna de esas condiciones.
...
- **Ventana, tramo y posición del eje viven en `simulation-window.helper.ts`** (`resolveTodayMonthKey` / `buildWindowMonths` / `computeHorizonEndMonth` / `effectiveStartMonth` / `buildMonthRange` / `axisPositionFor`), no dispersos en el service — reusarlos, no reimplementar la aritmética de RN-028.
- **`computeHorizonEndMonth(baseMonthKey)` toma un mes base genérico, no asume "hoy".** Calcula el `endMonth` de **un** tramo a partir de su propio arranque: al crear, `baseMonthKey` es el `startMonth` ya clampeado (nunca el mes en curso salvo que coincidan); en `GET /simulations/candidates`, es el `startMonth` hipotético del lote a previsualizar. Cada simulación tiene su propio tramo, anclado a su propio arranque — no hay un único horizonte global anclado siempre a "hoy".
- **Gotcha — el umbral de `computeHorizonEndMonth` mide un tramo distinto del que se simula, y está bien así.** El chequeo de "menos de 6 meses" que dispara la extensión a `base+6` cuenta los meses **estrictamente posteriores** a `baseMonthKey` (`base+1..diciembre`), mientras que el tramo simulable **incluye** `baseMonthKey` (`base..fin`). **No es un off-by-one**: son dos magnitudes distintas (el largo del tramo natural que decide si hace falta extender vs. el rango que efectivamente se deriva). "Arreglar" el umbral para que cuente desde `base` corre el corte de la extensión un mes.
...
- **Los `400` del body son tres**: `categoryIds` vacío, `categoryIds` con ids duplicados, o `startMonth` (si viene) que no matchea `/^\d{4}-\d{2}$/` — los resuelve la validación del DTO antes de tocar la DB. Un rechazo **por categoría** nunca sale como status de error: viaja en `failed` con `201`.
