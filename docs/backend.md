...
|--------|-----------|-------------|
| `movements` | `GET /movements`, `GET /movements/reports`, `GET /movements/reports/annual-unicos`, `GET /movements/reports/annual-cuotas`, `GET /movements/reports/annual-inflation-income`, `GET /movements/reports/annual-fijos` | Lista unificada del mes + serie de reportes agregada + grilla anual de Únicos + gantt anual de Cuotas + series anuales de Inflación vs Ingresos + serie histórica por gasto fijo (transacciones + recurrentes + cuotas) |
...
### `GET /movements/reports/annual-fijos?rangeMonths=&currency=&today=`
...
Una serie de **los últimos `rangeMonths` meses hasta el mes en curso**, **por gasto fijo**, para la card `fixed-evolution` (RF-REP-013), scopeado por `userId` del JWT. Contrato (params, shape `AnnualFijosResponse` / `FixedEvolutionLine` / `FixedEvolutionMonthPoint` / `FixedEvolutionExcludedLine`) en `docs/data-model.md`, §Contrato de reporte anual de Fijos. Reglas de negocio:
...
- **Mes base de la variación — el mes anterior al primer mes del rango.** La variación de `startMonth` compara contra ese mes previo, que **no** viaja en la serie, convertido con **la cotización de su propio mes**.
- **Cotizaciones e IPC se cargan por LISTA DE MESES del rango** (los meses del rango efectivo más el mes base), no por año calendario: el rango arranca en cualquier mes y puede cruzar varios años.
- **El mes en curso participa de la variación como cualquier otro** (asimetría deliberada con `annual-inflation-income`, que anula la variación del mes en curso). Un fijo es **determinístico** (RN-016): su monto del mes ya está definido, no es una medición incompleta como el total de ingreso. `nominalPct` / `adjustedPct` solo son `null` por falta de monto anterior (o de IPC, en la ajustada).
